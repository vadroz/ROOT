<%@ page import="rciutility.StoreSelect, ecommerce.EComSrlAsg, inventoryreports.PiCalendar, java.util.*"%>
<%
...
{
   response.sendRedirect("SignOn1.jsp?TARGET=EComSrlAsg.jsp&APPL=ALL&" + request.getQueryString());
}
else
{
    String sAuthStr = session.getAttribute("STORE").toString().trim();
    String sUser = session.getAttribute("USER").toString();
    EComSrlAsg itmasgn = new EComSrlAsg(sSelStr, sStatus, sStsFrDate, sStsToDate
...
<style>body {background:ivory;}
        ...
        td.DataTable1Y { background: yellow; padding-top:3px; padding-bottom:3px; padding-left:3px;
                       padding-right:3px; text-align:left; white-space:}
        td.DataTable1e { background: pink; padding-top:3px; padding-bottom:3px; padding-left:3px;
...
{ 
	  ...
	  document.all.dvItem.style.pixelLeft=document.documentElement.scrollLeft + 30;
...
{
	url = "EComSrlAsgSave.jsp?PackId=" + packid
	 + "&Action=RePrtPackId"
	;
...
function getPackId()
{
   ...
   var packid = 0;	 
   var url = "EComGetFedExPackId.jsp?Site=" + sel[0] + "&Order=" + sel[1] + "&Action=PostPackId";
   var xmlhttp;
...
{
   var panel = "<table cellPadding='0' cellSpacing='0' border=1>"
     + "<tr><td nowrap class='Small1' colspan=2>Status: " + snsts + " &nbsp; &nbsp; Str: " + aStr[aLine[i]] + " </td></tr>"
     + "<tr>"
       + "<td nowrap class='Small'>" + qtynm + "</td>"
       + "<td nowrap>"
          + "<select class='Small' name='selQty" + i + "S" + j
   ...
   if(newSts == "Picked")
   {
     panel += "<tr>"
       + "<td nowrap class='Small' colspan=2>"
         + "<input type='checkbox' name='Excl" + i + "S" + j + "' value='" + i + "' disabled>Add Sku to Exclusion"
         + "<br>Click&nbsp;"
   ...
   panel += "<tr>"
       ...
       + "<td nowrap class='Small'>"
          + "<input class='Small' name='Emp" + i + "S" + j + "' size=4 maxlength=4"
   ...
   if(newSts == "Picked")
   {
      panel += "<tr>"
           ...
           + "<td nowrap class='Small'>"
              + "<select class='Small' name='Reason" + i + "S" + j + "'>"
   ...
   panel += "<tr>"
       ...
       + "<td nowrap class='Small'><TextArea class='Small' name='txaNote" + i + "S" + j + "' cols=30 rows=3></TextArea></td>"
    + "</tr>"
    + "<tr>"
...
function chkExclSku(arg, newSts, j)
{
   ...
   if(newSts == "Shipped" || newSts == "Picked")
   {
      ...
      if(qty == 0){ document.all[exclnm].checked = true; document.all[exclnm].disabled = false;}
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor="moccasin">
    <TD vAlign=top align=left><B>Retail Concepts Inc.
        ...
        <a href="EComSrlAsgSel.jsp" class="small"><font color="red">Select</font></a>&#62;
        ...
        <%if(bSend 
            ...
            &nbsp; &nbsp; &nbsp; &nbsp;
            <a href="EComSrlAsgUnPick.jsp?Str=<%=sSelStr[0]%>&Sort=Div">Print Non-Picked</a>
            &nbsp; &nbsp; &nbsp; &nbsp;
...
<!-- ======================================================================= -->
       ...
       %>
         ...
         <tr class="DataTable0<%=sOrdClr%>">
            ...
            <td class="DataTable" id="tdOrd<%=i%>" nowrap rowspan=<%=iNumOfSrn%>><%=sOrd%>
            ...
            <td class="DataTable1<%if(bExpedited){%>E<%}%>"  nowrap rowspan=<%=iNumOfSrn%>><%=sShipMeth%> - <b><%=sShipMethNm%></b></td>
        ...
        %>
           ...
           <td class="DataTable" nowrap>              
             <%if(bPack && !sPickQty.equals("") && !sPickQty.equals("0")){%><a href="javascript: rtvPickedItem('<%=i%>');"><%=sPackId%></a><%}
             else{%><%=sPackId%><%}%>
