...
//==============================================================================
function showStrKioSls(obj, stronly, kiosk, strkio, ecomsls)
{
    var html = "<table border='0' width='100%' cellPadding='0' cellSpacing='0'>"
      + "<tr style='font-size:10px'>"
           + "<td>Store Sales:</td>"
 ...
 <table border="0" width="100%" height="100%" cellPadding="0" cellSpacing="0">
  ...
  <table class="DataTable" cellPadding="0" cellSpacing="0">
    ...
    <tr>
      ...
      <th class="DataTable">Sls<br>1.88%</th>
      <!--th class="DataTable">MS</th-->
      <%if(sGrp.equals("MGR") || sGrp.equals("MGRPI")){%>
         <!--th class="DataTable">Aud</th-->
         <th class="DataTable">Pay<br>.94%</th>
         <th class="DataTable">PvO<br>1.88%</th>
         <th class="DataTable">Surv<br>.78%</th>
      <%}%>
      <th class="DataTable">Tot<br>5.47%</th>
    </tr>
<!------------------------------- Detail Data --------------------------------->
    ...
    <%String sSvReg = " ";
      ...
      <tr class="DataTable">
         ...
         <td class="DataTable" id="PSMn0S0<%=i%>" onMouseOver="showStrKioSls(this, '<%=sStrSKStr[i][0]%>', '<%=sStrSKKio[i][0]%>', '<%=sStrSKSls[i][0]%>', '<%=sStrSKEcom[i][0]%>');" onMouseOut="hideAlwTmc();" nowrap>$<%=sSales[0][i]%></td>
         <td class="DataTable" nowrap>$<%=sPlanVar[0][i]%></td>
         <th class="Divdr">&nbsp;</th>
         <td class="DataTable" id="PSMn1P0<%=i%>" nowrap>$<%=sPlan[1][i]%></td>
         <td class="DataTable" id="PSMn1S0<%=i%>" onMouseOver="showStrKioSls(this, '<%=sStrSKStr[i][1]%>', '<%=sStrSKKio[i][1]%>', '<%=sStrSKSls[i][1]%>', '<%=sStrSKEcom[i][1]%>');" onMouseOut="hideAlwTmc();" nowrap>$<%=sSales[1][i]%></td>
         <td class="DataTable" nowrap>$<%=sPlanVar[1][i]%></td>
         <th class="Divdr">&nbsp;</th>
         <td class="DataTable" id="PSMn2P0<%=i%>" nowrap>$<%=sPlan[2][i]%></td>
         <td class="DataTable" id="PSMn2S0<%=i%>" onMouseOver="showStrKioSls(this, '<%=sStrSKStr[i][2]%>', '<%=sStrSKKio[i][2]%>', '<%=sStrSKSls[i][2]%>', '<%=sStrSKEcom[i][2]%>');" onMouseOut="hideAlwTmc();" nowrap>$<%=sSales[2][i]%></td>
         <td class="DataTable" nowrap>$<%=sPlanVar[2][i]%></td>
         <th class="Divdr">&nbsp;</th>
         <td class="DataTable" id="PSMn3P0<%=i%>" nowrap>$<%=sPlan[3][i]%></td>
         <td class="DataTable" id="PSMn3S0<%=i%>" onMouseOver="showStrKioSls(this, '<%=sStrSKStr[i][3]%>', '<%=sStrSKKio[i][3]%>', '<%=sStrSKSls[i][3]%>', '<%=sStrSKEcom[i][3]%>');" onMouseOut="hideAlwTmc();" nowrap>$<%=sSales[3][i]%></td>
          ...
          <%for(int j=0; j < 7; j++ ) {%>
            ...
            <%for(int k=0; k < 4; k++ ) {%>
              ...
              <%if(j==0) {%>
                ...
                <td class="DataTable" onMouseOver="showStrKioSls(this, '<%=sRegSKStr[iRow-1][k]%>', '<%=sRegSKKio[iRow-1][k]%>', '<%=sRegSKSls[iRow-1][k]%>', '<%=sRegSKEcom[iRow-1][k]%>');" onMouseOut="hideAlwTmc();" id="PSMn<%=k%>SR<%=iRow-1%>">$<%=sRepTotVal[k][iRow-1][1]%></td>
              <%}%>
              <%if(j==3) {%>
                ...
                <span id="colPayPrc"><%=sRegAlwBdgPayVsSlsPrc[iRow-1][k]%>%</span><span id="colPayHrs"><%=sRegAlwBdgHrs[iRow-1][k]%></span><span id="colPayAmt">$<%=sRegAlwBdgPay[iRow-1][k]%></span></td>
       ...
       <%for(int j=0; j < 7; j++ ) {%>
         ...
         <%for(int k=0; k < 4; k++ ) {%>
            ...
            <%if(j==0) {%>
                ...
                <td class="DataTable" onMouseOver="showStrKioSls(this, '<%=sRepSKStr[k]%>', '<%=sRepSKKio[k]%>', '<%=sRepSKSls[k]%>', '<%=sRepSKEcom[k]%>');" onMouseOut="hideAlwTmc();" id="PSMn<%=k%>ST"><%=sRepTotVal[k][4][1]%></td>
            <%}%>
            <%if(j==3) {%>
...
<!---------------------------- end of Report Totals ------------------------------>
 ...
 <p  align="left" style="font-size:12px">
   ...
   *To earn the 'Mystery Shop' and 'Store Audit' category bonus you must achieve an average score of at least
   ...
   <%if(sGrp.equals("MGR")) {%>*To see Incentive Plan Table click <a href="IncPlanArchive/Incentive_Plan_Table_Q4_2014.pdf" target="_blank">here</a><%}
   ...
   <br>*To earn Ecomm sales to being added, you must have at least 75% or greater order fulfillment rate.
 </span>
 <!----------------------- end of table ------------------------>
