<%@ page import="payrollreports.MtxStrMonBfdgAvg, java.util.*, java.sql.*"%>
<%
...
else
{
    ...
    MtxStrMonBfdgAvg bdgmtx = new MtxStrMonBfdgAvg(sMon, sUser);
    String sApplyYear = bdgmtx.getApplyYear();
    ...
    String [] sMonNm = new String[]{"April", "May", "June", "July", "August", "September"
      , "October", "November", "December", "January", "February", "March", "Annual Worksheet" };
    int iMon = Integer.parseInt(sMon) - 1;
%>
...
<style>body {background:ivory;font-family: Verdanda}
        ...
        div.dvMonSel { position:absolute;  background-attachment: scroll;
              border: lightgray outset 1px; width:160; background-color:#ccffcc; z-index:10;
...
var MonNm = ["April", "May", "June", "July", "August", "September", "October", "November"
     , "December", "January", "February", "March"  ];
var ApplyYear = "<%=sApplyYear%>";
//==============================================================================
...
function setMonSel()
{
   ...
   for(var i=0; i < 12; i++)
   {
      html += "<br> &nbsp; &nbsp; &nbsp; &nbsp; <a href='MtxStrMonBfdgAvg.jsp?Mon=" + (i+1) + "'>" + MonNm[i] + "</a>";
   }
...
//============================================================================================
// Apply Matrix for selected store and month to Budget Averages Wages regular pay caregories
//============================================================================================
function setApplyMtx()
{
   var hdr = "Apply Matrix to Budget Averages Regular Pay";
   var html = "<table border=0 width='100%' cellPadding=0 cellSpacing=0 id='tblHrsEarn'>"
     + "<tr>"
       + "<td class='BoxName' nowrap>" + hdr + "</td>"
       + "<td class='BoxClose' valign=top>"
...
function popApplyMtx()
{
    ...
    panel += "<tr class='DataTable1'>"
       + "<td class='DataTable1'>"
       + "<span style='color:darkred; font-size:14px'>The Average wages will be applied to "
       + CurMonNm + "/" + ApplyYear + " budget</span>"
    panel += "</td></tr>"
...
function setCopyMtx()
{
   ...
   for(var i=0; i < 12; i++)
   {
	   ...
	   if(i+1 > eval(Mon))
	   {
		   document.getElementById("Mon" + i).checked = true;
	   }
   }
}
//==============================================================================
...
function popCopyMtx()
{
    ...
    {
       if(i > 0 && i%10 ==0){ panel += "</tr><tr class='DataTable1'>"}
       panel += "<td class='DataTable1'><input type='checkbox' name='Str' value='" + Str[i] + "' checked>" + Str[i] + "</td>"
    }
    panel += "</table>"
    panel += "</td></tr>"
    ...
    for(var i=0; i < Grp.length; i++)
    {
       if(i > 0 && i%6 ==0){ panel += "</tr><tr class='DataTable1'>"}
       panel += "<td class='DataTable1'><input type='checkbox' name='Grp' value='" + Grp[i] + "' checked>" + Grp[i] + "</td>"
    }
    panel += "</table>"
    panel += "</td></tr>"
    
    //populate applied month    
    panel += "<tr class='DataTable1'>"
     + "<td class='DataTable1'><u><b>Appiled to Month(s):</b></u>"
     + "<table><tr class='DataTable1'>"

    for(var i=0; i < 12; i++)
    {
       if(i > 0 && i%6 ==0){ panel += "</tr><tr class='DataTable1'>"}
       panel += "<td class='DataTable1'><input type='checkbox' name='Mon" + i + "' value='" + (i+1) + "'>" + MonNm[i] + "</td>"
    }
    panel += "</table>"
    panel += "</td></tr>"
    ...
    panel += "<tr class='DataTable3'>"
          + "<td class='DataTable'>"
             + "<button onClick='validCopy();' class='Small'>Copy</button> &nbsp; &nbsp;"
             + "<button onClick='hidePanel();' class='Small'>Close</button>"
          + "</td>"
       + "</tr>"
     + "</table>"
...
function validCopy()
{
	...
	for(var i=0; i < 12; i++)
	{
		if(document.getElementById("Mon" + i).checked){ monval[i] = 'Y'; monsel = true;}
		else{ monval[i] = 'N'; }
	}
	...
	if(error){alert(msg)}
	else{ sbmCopyMtx(strval,grpval,monval)}
}
//==============================================================================
//submit - copy
//==============================================================================
function sbmCopyMtx(str, grp, mon)
{
	var url = "MtxBdgAvgSave.jsp?Action=COPY_MON_MTX_BDG"
	  + "&CurMon=" + Mon
...
{
   var url = "MtxBdgAvgSave.jsp?Action=APPLY_MON_MTX_BDG"
   for(var i=0; i < str.length; i++)  {  url += "&Str=" + str[i]; }
   for(var i=0; i < mon.length; i++)  {  url += "&Mon=" + mon[i]; }
    ...
    <table border="0" width="100%" cellPadding="0" cellSpacing="0">
     ...
     <tr bgColor="ivory" style="position: relative; top: expression(this.offsetParent.scrollTop);">
       <td ALIGN="center" VALIGN="TOP" width="10%">
         ...
         <br>Store Monthly Budget Average Wage Matrix
         <br>Month: <span style="color:darkred;"><%=sMonNm[ Integer.parseInt(sMon) - 1]%></span>
         </b>
         <br>
         <%if(!sInFrame.equals("Y")) {%>
            ...
            <a href="MtxStrBfdgAvg.jsp" class="small"><font color="red">All Stores Budget</font></a>&#62;
            <font size="-1">This Page.</font>&nbsp;&nbsp;&nbsp;&nbsp;
            <a class="small" href="BfdgAvgWage.jsp?Year=2013&Month=<%=sMon%>&MonName=<%=sMonNm[iMon]%>&Str=3&Str=4&Str=5&Str=8&Str=10&Str=11&Str=20&Str=28&Str=29&Str=30&Str=35&Str=40&Str=42&Str=45&Str=46&Str=50&Str=55&Str=61&Str=63&Str=64&Str=66&Str=75&Str=82&Str=86&Str=87&Str=88&Str=89&Str=90&Str=92&Str=93&Str=96&Str=98&Over=10&Under=10" target="_blank">Budget Budget group average wages break-up</a>
            &nbsp;&nbsp;&nbsp;&nbsp;
         <%}%>
         <a href="javascript: setApplyMtx()" class="small">Apply Matrix</a>
         &nbsp; &nbsp; &nbsp;
         <a href="javascript: setCopyMtx()" class="small">Copy</a>
       </td>
     </tr>
     </thead>
