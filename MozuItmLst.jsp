...
<style>body {background:ivory;}
        ...
        td.DataTable2g { background: lightgreen; padding-top:3px; padding-bottom:3px; padding-left:3px;
                       padding-right:3px; text-align:right; white-space:}
        td.DataTable1p { background: pink; color:black; padding-top:3px; padding-bottom:3px; padding-left:3px;
                       padding-right:3px; text-align:left; white-space:}
        ...
        button.Small1 { padding:0px; margin:0px; font-size:9px }

		div.dvLog { position:absolute; top: expression(this.offsetParent.scrollTop);
               left: expression(this.offsetParent.scrollLeft);
               background-attachment: scroll; border: MidnightBlue solid 1px; width:150;
...
function chgItem(i,cls,ven,sty,clr,siz)
{
   ...
   document.all.dvItem.style.left = ColPos[0] + 30;
   document.all.dvItem.style.top = ColPos[1] + 15;
...
//check if order is paid off
   ...
   var url = "MozuItmSave.jsp?Site=" + Site + "&Cls=" + cls + "&Ven=" + ven + "&Sty=" + sty + "&Clr=" + clr + "&Siz=" + siz
...
function vldDownl(i,dcls,dven,dsty,dclr,dsiz)
{
   ...
   if (NumOfItm > 1)
   {
	   clr = document.all.tdColor[i].innerHTML.trim();   
	   nclr = document.all.tdNormClr[i].innerHTML; 
	   siz = document.all.tdSize[i].innerHTML.trim();
	   web = document.all.tdWebN[i].innerHTML;
   }   
   else
   {
	   clr = document.all.tdColor.innerHTML.trim();   
	   nclr = document.all.tdNormClr.innerHTML; 
	   siz = document.all.tdSize.innerHTML.trim(); 
	   web = document.all.tdWebN.innerHTML;
   }
   
   if(clr == ""){ error = true; msg += "\nPlease add Color."; }
   else if(clr == "No Color" && siz == "No Size"){ error = true; msg += "\nItem must has color or size."; }
...
function colUpdate()
{
   ...
   document.all.dvItem.style.left= ColPos[0] + 30;;
   document.all.dvItem.style.top= ColPos[1] + 20;;
   document.all.dvItem.style.visibility = "visible";
   document.all.dvItem.scrollIntoView(true);
   ...
   if(SelCol=="CLRNM")
   {
	   if(Site == "11961"){ rtvAttrLst("tenant~color"); }
	   else if(Site == "25861"){ rtvAttrLst("Tenant~Color"); }
...
{	
	if(attr.toLowerCase() != "tenant~color" && attr.toLowerCase() != "tenant~size"){OptionSel = opts;}
	...
	if(attr.toLowerCase() == "tenant~color" && document.all.Col.value.trim() == ""){ setPreSelIpOpt(attr); }
	else if(attr.toLowerCase() == "tenant~size" && document.all.Col.value.trim() == ""){ setPreSelIpOpt(attr); }
...
function loadSelPanel(opts, optnms, attr)
{
	...
	for(var i=0; i < opts.length; i++)
	{
  		panel += "<tr id='trOptSel" + i + "'>"
  	 	if(attr == null || attr.toLowerCase() != "tenant~color" && attr.toLowerCase() != "tenant~size")
  	 	{	
...
{	
	...
	{  
		for(var i=0; i < OptionSel.length; i++)
		{
  			if(   attr.toLowerCase() != "tenant~color" && OptionSel[i].toUpperCase().indexOf(opt) >= 0
  			   || attr.toLowerCase() == "tenant~color" && OptionSel[i].toUpperCase().indexOf(opt) == 0	) 
  			{ 
  				fnd = true;
...
{
	var parent = "NONE";
	if(attr.toLowerCase() != "tenant~brand" && attr.toLowerCase() != "tenant~gender" 
	    && attr.toLowerCase() != "tenant~normalized-color")
	{
		...
		for(var i=0, j=0; i < NumOfItm; i++)
		{
			if(SelCell[i] != null && SelCell[i])
	    	{
	    		if (NumOfItm > 1)
	        	{	
	    			parent = document.all.tdItem[i].innerHTML.substring(0, 19);
	    			break;
	        	}
	    		else
	    		{
	    			parent = document.all.tdItem.innerHTML.substring(0, 19);
	    			break;
	    		}
	    	}
		}
...
function crtNewOption(col, colnm)
{
	   ...
	   document.all.dvOpt.style.left = ColPos[0] + 30;
	   document.all.dvOpt.style.top = ColPos[1] + 20;
...
function sbmNewOpt(col, optnm)
{
	...
	else if(col=="CLRNM"){ if(Site=="12020") { attr = "Tenant~Color"; } else { attr = "tenant~color"; }}
	else if(col=="NORMCLR"){ attr = "tenant~normalized-color"; }
	else if(col=="SIZNM"){ if(Site=="12020") { attr = "Tenant~Size"; } else { attr = "tenant~size"; } }
	...
	var html = "<form name='frmAddOpt'"
   	 + " METHOD=Post ACTION='MozuCrtOptOrPropValue.jsp'>"
   	 + "<input class='Small' name='Site'>"
   	 + "<input class='Small' name='Attr'>"
   	 + "<input class='Small' name='ProdType'>"
   	 + "<input class='Small' name='PtId'>"
   	 + "<input class='Small' name='OptNm'>"
   	 + "</form>"
   	 ;
	...
	window.frame1.document.all.Attr.value=attr;
	window.frame1.document.all.ProdType.value=SelProdType;
	window.frame1.document.all.PtId.value=SelProdTypeId;
	window.frame1.document.all.OptNm.value=optnm;
	
	window.frame1.document.frmAddOpt.submit();	
...
{	
	...
	document.all.dvProgBar.style.left = ColPos[0] + 30;
	document.all.dvProgBar.style.top = ColPos[1] + 20;
...
function sbmColumn(col)
{
   ...
   //document.all.dvItem.innerHTML = column;
   var column = document.all.Col.value;
   var column1 = column;
   
   while(column1.indexOf('"') >= 0)
   {
	   column1 = column.replace('"', '%22');
   }
   ...
   document.all.dvItem.style.left = ColPos[0] + 30;
   document.all.dvItem.style.top = ColPos[1] + 10;
   document.all.dvItem.style.visibility = "visible";
   ...
   for(var i=0, j=0; i < NumOfItm; i++)
   {
      if(SelCell[i] != null && SelCell[i])
      {
        if (NumOfItm > 1)
        {
           ...
           clr = document.all.tdItem[i].innerHTML.substring(20, 24);
           if(document.all.tdItem[i].innerHTML.indexOf("<SPAN") > 0)
           {
        	   siz = document.all.tdItem[i].innerHTML.substring(25, 30);
           }
           else { siz = document.all.tdItem[i].innerHTML.substring(25); }
        }
        else
        {
           ...
           clr = document.all.tdItem.innerHTML.substring(20, 24);
           if(document.all.tdItem.innerHTML.indexOf("<SPAN") > 0)
           {
        	   siz = document.all.tdItem.innerHTML.substring(25, 30);
           }
           else { siz = document.all.tdItem.innerHTML.substring(25); }
        }

        var url = "MozuItmSave.jsp?Site=" + Site + "&Cls=" + cls + "&Ven=" + ven + "&Sty=" + sty + "&Clr=" + clr + "&Siz=" + siz
            + "&SavMNF=" + savven

        if(col=="DESC") url += "&Desc=" + column1 + "&Action=UPDDESC";
        else if(col=="CATEG") url += "&Categ=" + column1 + "&Action=UPDCATEG";
        else if(col=="CLRNM") url += "&ClrName=" + column1 + "&Action=UPDCLRN";
        else if(col=="NORMCLR") url += "&NormClr=" + column1 + "&Action=UPDNCLR";
        else if(col=="SIZNM") url += "&SizName=" + column1 + "&Action=UPDSIZN";
        else if(col=="MNF") url += "&Mnf=" + column1 + "&Action=UPDMNF";
        else if(col=="MODEL") url += "&Model=" + column1 + "&Action=UPDMODEL";
        else if(col=="MDLYR") url += "&MdlYear=" + column1 + "&Action=UPDMDLYR";
        else if(col=="GENDER") url += "&Gender=" + column1 + "&Action=UPDGENDER";
        else if(col=="MAP") url += "&Map=" + column1 + "&Action=UPDMAP";
        else if(col=="NOMAP") url += "&NoMap=" + column1 + "&Action=UPDNOMAP";
        else if(col=="MAPDT") url += "&MapDt=" + column1 + "&Action=UPDMAPDT";
        else if(col=="LIVE") url += "&Live=" + column1 + "&Action=UPDLIVE";
        else if(col=="PRDTY") url += "&PrdTy=" + column1 + "&Action=UPDPRDTY";
...
<TABLE height="100%" width="100%" border=0  cellPadding="0" cellSpacing="0">
  ...
  <TR bgColor=moccasin>
    <TD vAlign=top align=center>
        ...
        <a href="javascript: clrColSel()">Clear</a>&nbsp;&nbsp;&nbsp;&nbsp;<br>
...
<!-- ======================================================================= -->
       <table border=1 cellPadding="0" cellSpacing="0" id="tbRtvEnt">
         <tr class="DataTable">
             ...
             <th id="thDesc" class="DataTable" rowspan=2 nowrap >
                <a href="javascript: resort('DESC');">Item<br>Description</a>
             </th>
             ...
             <th id="thSkiChlt" class="DataTable"  >On Hand<br>Qty</th>
             <th class="DataTable" rowspan=2 >VDS</th>
       ...
       <%for(int i=0; i < iNumOfItm; i++ )
         {
            ...
            String sMultUpc = itmLst.getMultUpc();
            String sVdsQty = itmLst.getVdsQty();
            String sHasVDs = itmLst.getHasVds();
            ...
            boolean bNoClrSiz = sClrName.equals("No Color") && sSizName.equals("No Size")
            		|| sClrName.equals("") && sSizName.equals("");
       ...
       %>
         <tr id="trItem" class="DataTable<%if(sOldItem.equals("1")){%>2<%}%>">
            ...
            <%}%>
            <td id="tdLstDwn" class="DataTable1" nowrap><%=sOrigDown%></td>
            ...
            <td id="tdColor" <%if(bDwnLoad){%>onMouseDown="setSearchClr('<%=sPoClrNm%>','<%=sIpClrNm%>'); selColumnCell(<%=i%>, '<%=sCls%>','<%=sVen%>','<%=sSty%>','<%=sClr%>','<%=sSiz%>', 'CLRNM')"<%}%> class="DataTable1" 
            	<%if(bNoClrSiz){%>style="background:red;"<%}%> nowrap><%=sClrName%>
            </td>
            ...
            <td id="tdSize" <%if(bDwnLoad){%>onMouseDown="SelIpOptId='<%=sIpSizNm%>'; selColumnCell(<%=i%>, '<%=sCls%>','<%=sVen%>','<%=sSty%>','<%=sClr%>','<%=sSiz%>', 'SIZNM')"<%}%> class="DataTable1"
              <%if(bNoClrSiz){%>style="background:red;"<%}%>  nowrap><%=sSizName%>
            </td>
            ...
            <td id="tdOnhand" class="DataTable2<%if(sHasVDs.equals("Y")){%>g<%}%>" nowrap><%=sVdsQty%></td>
            
            <td class="DataTable1<%if(sMultUpc.equals("Y")){%>p<%}%>" id="tdIpSizNm<%=i%>" nowrap>
            	<%=sUpc%> 
            </td>
          </tr>
       <%}%>
