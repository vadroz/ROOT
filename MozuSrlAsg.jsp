...
{
   response.sendRedirect("SignOn1.jsp?TARGET=MozuSrlAsg.jsp&APPL=ALL&" + request.getQueryString());
}
else
{
...
function popPickedItem(line)
{
	...
	panel += "<tr class='DataTable4'>"
		 + "<td nowrap class='Medium'colspan=10>Employee: "
		    + "<input class='Small' name='Emp' size=10 maxlength=10>"
		 + "</td>"
	  + "</tr>"
 ...
 if(arg < aSelOrd.length)
 {
     ...
     if(isIE || isSafari)
 	 {
         ...
         window.frame1.document.all.Emp.value = sel[4];
   		 ...
   		 window.frame1.document.all.LastArg.value="N";
   		 if(arg == (aSelOrd.length - 1)){window.frame1.document.all.LastArg.value="Y";}
     ...
     else
     {
         ...
         window.frame1.contentDocument.forms[0].Emp.value = sel[4];
   		 ...
   		 window.frame1.contentDocument.forms[0].LastArg.value="N";
   		 if(arg == (aSelOrd.length - 1)){window.frame1.contentDocument.forms[0].LastArg.value="Y";}    	 
...
{
	url = "MozuSrlAsgSave.jsp?PackId=" + packid
	 + "&Action=RePrtPackId"
	;
...
function popStsQtyPanel(line, newSts, pickqty)
{
  ...
  var panel = "Scan Item: <input id='ScanSku' onkeypress='if (window.event.keyCode == 13) { searchScan(this.value.trim()); }' class='Small' maxlength=14 size=17>"
...
function popSngSku(i, qtynm, newSts, snsts, j, pickqty)
{
   ...
   panel += "<tr class='DataTable'>"
       ...
       + "<td nowrap  class='DataTable1'>"
          + "<input class='Small' name='Emp" + i + "S" + j + "' id='Emp" + i + "S" + j + "' size=10 maxlength=10"
   ...
   if(newSts == "Picked")
   {
      panel += "<tr class='DataTable'>"
           ...
           + "<td nowrap class='DataTable1'>"
              ...
              + "</select>"
              + "<span id='spnRtvStr" + i + "S" + j + "' style='display:none;'>To Store:&nbsp;"
...
function chkExclSku(arg, newSts, j)
{
   ...
   var reas =  document.getElementsByName("Reason" + arg + "S" + j)[0]; 
   ...
   if(newSts == "Shipped" || newSts == "Picked")
   {
      ...
      if(qty == 1)
      {
    	  reas.selectedIndex = 1;  
      }
      else 
      {
    	  reas.selectedIndex = 0; 
      }
   }
}
//==============================================================================
...
function setRtvStr(sel,i,j)
{
	...
	var span = document.getElementById("spnRtvStr" + i + "S" + j);
	
    if(User.indexOf("w7") >=0 && (reas=="RTV" || reas == "Soiled/Damaged" || reas == "Missing Pieces"))
    {
    	span.style.display = "block";
    	document.getElementsByName("RtvStr" + i + "S" + j)[0].selectedIndex = 1; 
    }
    else if(span != null ){ span.style.display = "none"; }
}
//==============================================================================
...
function searchScan(search)
{
   ...
   if(!sku && !upc)
   {
      ...
      if( sresult == "")
      {
         ...
         {
        	 var lin = aLine[i];
        	 var asku = aSku[aLine[i]] + " ";
        	 asku = asku.trim();
             //if(aSku[aLine[i]].substring(3) == search){ sku = aSku[aLine[i]]; break; }
             if(asku.substring(3) == search){ sku = asku; break; }
         }
      ...
      if(aOrd[i] == search)
      {
         ...
         for(var j=0; j < aSrln[i].length;j++)
         {
            if(newSts == "Picked" && aSts[i][j] == "Printed"
               || newSts == "Picked" && aSts[i][j] == "Resolve"
...
function sbmStrPickSendQty(arg)
{
  	...
  	else
	{
		window.frame1.contentDocument.forms[0].Site.value = sel[0];
	      ...
	      window.frame1.dcontentDocument.forms[0].Reas.value=sel[7];
...
function setStatus(site, ord, sku, sts, obj)
{
  ...
  var html = "<table cellPadding='0' cellSpacing='0'>"
    ...
    + "<tr>"
       + "<td nowrap class='Small'>Employee Number: </td>"
       + "<td nowrap class='Small'><input class='Small' id='Emp' size=10 maxlength=10></td>"
    + "</tr>"
...
function restart(arg)
{
   ...
   {
	  if(SbmFunc == "sbmStrPickSendQty") { sbmStrPickSendQty(arg); }
	  else if(SbmFunc == "sbmPostPackId") { sbmPostPackId(arg); }
   }
   else
   {
      url = "MozuSrlAsg.jsp?Sort=<%=sSort%>"
      for(var i=0; i < SelStr.length;  i++) { url += "&Str=" + SelStr[i]; }
      for(var i=0; i < Sts.length;  i++) { url += "&Sts=" + Sts[i]; }
      url += "&StsFrDate=" + StsFrDate;
      url += "&StsToDate=" + StsToDate;
...
function ValidateStrSts(site,ord,sku,str)
{
    ...
    if(error){ document.all.tdErrorAll.innerHTML=msg; }
...
<!-------------------------------------------------------------------->
<iframe  id="frame0" name="frame0" src=""  height="0" width="0"></iframe>
<iframe  id="frame1" name="frame1" src=""  height="0" width="0"></iframe>
<iframe  id="frame2" name="frame2" src=""  height="0" width="0"></iframe>
<!-------------------------------------------------------------------->
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor="moccasin">
    <TD vAlign=top align=left><B>Retail Concepts Inc.
        ...
        <a href="MozuSrlAsgSel.jsp" class="small"><font color="red">Select</font></a>&#62;
...
<!-- ======================================================================= -->
       <table class="DataTable" cellPadding="0" cellSpacing="0">
         <tr class="DataTable">
             ...
             <th class="DataTable" id="cellFold1">PO#</th>
             ...
             <th class="DataTable" id="cellFold2"><a href="javascript: resort('VEN')">Vendor Name</a></th>
       ...
       <%for(int i=0, iHdg=0; i < iNumOfItm; i++, iHdg++ )
         {
            ...
            String sPoNum = itmasgn.getPoNum();
       ...
       %>
         ...
         <tr class="DataTable0<%=sOrdClr%>">
            ...
            <td class="DataTable1" id="cellFold1" nowrap rowspan=<%=iNumOfSrn%>><%=sPoNum%></td>
        ...
        %>
           <script language="javascript">
              ...
              aCnl1User[<%=i%>][<%=j%>] = "";
              <%if(!sCnf1Emp.equals("")){%>
                  aCnl1User[<%=i%>][<%=j%>] = "<%=sCnf1Emp.substring(0,10)%>";
              <%}%>
           ...
           <td class="DataTable2" id="tdCnl1<%=i%>" nowrap>              
              <%if(!sCnf1Dt.equals("01/01/0001")){%><%if(!sCnf1Emp.equals("")){%><%=sCnf1Emp.substring(0, 10)%><%}%><%}
