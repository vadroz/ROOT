<%@ page import="payrollreports.PsMonBudgAllStr,  payrollreports.SetWeeks, java.util.*, java.text.*"%>
<%
  ...
  {
     response.sendRedirect("SignOn1.jsp?TARGET=PsMonBudgAllStr.jsp&APPL=" + sAppl + "&" + request.getQueryString());
  }
   else {
  ...
  int iNumOfStr = 0;
   ...
   setbud = new PsMonBudgAllStr(sWeekEnd, sSort, sInclReg, sUpToCurr, sExclude70, sUser);
   ...
   String [] sBaseWk = SetWk.getBaseWk();
   String [] sBsWkName = SetWk.getBsWkName();
...
function submitForm()
{
   ...
   var SbmString = "PsMonBudgAllStr.jsp"
         + "?MONBEG=" + selMonth
         + "&WEEKEND="
...
{
  var df = document.forms[0];
  var sbmString = "PsMonBudgAllStr.jsp?MONBEG=" + Month
                + "&WEEKEND=" + WeekEnd
                + "&SORT=<%=sSort%>"
                + "&UPTOCURR=" + upto
                + "&GROUP=<%=sGroup%>"
...
function showActAvgVarAll()
{
   ...
   var url = "PsActAvgVarAllStr.jsp?Store=ALL&From=BEGWEEK&To=<%=sWeekEnd%>&AllwBdg=" + allowb;
             ...
             <%for(k=0;  i < iNumOfStr && k < iReg[j]; i++, k++){%>
               ...
               <%}%>
                 <td class="DataTable" nowrap>
                    <a href="PsWkSched.jsp?STORE=<%=sStore[i]%>&STRNAME=<%=sStoreName[i]%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>">
                              <%=sStore[i]%></a></td>
                 ...
                 <th class="DataTable"><a href="BfdgSchActWk.jsp?Store=<%=sStore[i]%>&StrName=<%=sStoreName[i]%>&Wkend=<%=sWeekEnd%>" target="_blank">AB</a></h>
                 ...
                 <%if(!sActual.equals("0")){%>
                    <th class="DataTable"><a href="PsActAvgVar.jsp?Store=<%=sStore[i]%>&StrNm=<%=sStoreName[i]%>&From=BEGWEEK&To=<%=sWeekEnd%>" target="_blank">V</a></th>
                 <%}
