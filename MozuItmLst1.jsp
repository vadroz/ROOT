...
function popHistDownLog(item, time, errflg, error)
{
	...
	for(var i=0; i < item.length; i++)
	{
		panel += "<tr class='DataTable'>"
	     	+ "<td class='DataTable1' nowrap width='140'>" + item[i] + "</td>"
...
function colUpdate()
{
   ...
   //if(SelCol=="CLRNM"){ rtvAttrLst("tenant~color"); document.all.Col.readOnly = true;}
   if(SelCol=="CLRNM"){  document.all.Col.readOnly = true; setAttrUpd("tenant~color"); }
...
{  
  ...
  if(col == "LIVE")
  {
     ...
     panel += "<button class='Small' name='Down' onClick='setDate(&#34;DOWN&#34;, &#34;Col&#34;)'>&#60;</button>"
           ...
           + "<a href='javascript:showCalendar(1," + mon + "," + year + ", getLeftScreenPos() + 300, getTopScreenPos() + 220, document.all.Col)' >"
...
//show otpion selection list
//==============================================================================
function setAttrUpd(attr)
{		
	var html = "<table border=0 width='100%' cellPadding=0 cellSpacing=0 style='font-size:10px;'>"
	  + "<tr><td nowrap>"   
	    + "<input name='FndOpt' class='Small' size=50 maxlength=50>&nbsp;"	    
  	    + "<button onclick='checkAttr(&#34;" + attr + "&#34;)' class='Small'>Check</button>"
  	    + "<br><span id='spnError' class='Small' style='color:red;'></span>"
  	  + "</td></tr>"
	+ "</table>"
	
	document.all.dvOptSel.innerHTML = html;
	document.all.dvOptSel.style.display = "block";
	document.all.dvOptSel.style.height="40px"; 
	
	if(SelIpOptId != null && SelIpOptId != "" && document.all.Col.value == "")
	{
		document.all.Col.value = SelIpOptId;
		document.all.FndOpt.value = SelIpOptId;
		checkAttr(attr);
	}
}
//==============================================================================
...
function checkAttr(attr)
{
	...
	var error = false;	
	document.all.spnError.innerHTML = "";
	document.all.FndOpt.style.background = "white";
	
	var url = "MozuChkAttrByName.jsp?Attr=" + attr
		+ "&OptNm=" + opt
		+ "&ProdId=" + SelProdTypeId
	;
	...
	xmlhttp.onreadystatechange=function()
	{
		...
		{			
		   var  resp = xmlhttp.responseText;
		   error = parseElemArr(resp, "Error") == "true";
		   if(!error)
		   {
		   		optId = parseElemArr(resp, "OptId");
		   		opt = parseElemArr(resp, "OptVal");
		   		document.all.Col.value = opt;
		   		document.all.FndOpt.value = opt;
		   		document.all.FndOpt.style.background = "lightgreen";		   		
		   }		    
		   else
		   {
			  document.all.spnError.innerHTML = "Attribute is not found.";
			  document.all.FndOpt.style.background = "pink";
		   }
		}
	}
	xmlhttp.open("GET",url,false); // asynchronize with this apps
	xmlhttp.send();
...
function crtNewOption(col, colnm)
{
	   ...
	   document.all.dvOpt.style.left= getLeftScreenPos() + 400;
...
{	
	...
	document.all.dvProgBar.style.left= getLeftScreenPos() + 400;
	document.all.dvProgBar.style.top= getTopScreenPos() + 300;
   ...
   for(var i=0, j=0; i < NumOfItm; i++)
   {
      if(SelCell[i] != null && SelCell[i])
      {
        if (NumOfItm > 1)
        {
           ...
           {
        	   siz = document.all.tdItem[i].innerHTML.substring(20, 24);
           }
           else { siz = document.all.tdItem[i].innerHTML.substring(20); }
        }
        else
        {
           ...
           {
        	   siz = document.all.tdItem.innerHTML.substring(20, 24);
           }
           else { siz = document.all.tdItem.innerHTML.substring(20); }
        }
...
function reuseFrame()
{
   if (SbmLoop < SbmQty)
   {
      ...
      // rotate used frames
      LastFrame += 1;
      if(LastFrame >= window.frames.length){ LastFrame = 0; } 
      window.frames[LastFrame].location.href= SbmCmd[SbmLoop];
      SbmLoop++;
   }
   else
   {
...
<iframe  id="frame3"  src=""  height="0" width="0"></iframe>
<iframe  id="frame4"  src=""  height="0" width="0"></iframe>
<iframe  id="frame5"  src=""  height="0" width="0"></iframe>
<iframe  id="frame6"  src=""  height="0" width="0"></iframe>
<iframe  id="frame7"  src=""  height="0" width="0"></iframe>
<iframe  id="frame8"  src=""  height="0" width="0"></iframe>
<iframe  id="frame9"  src=""  height="0" width="0"></iframe>
<!-------------------------------------------------------------------->
...
<!-- ======================================================================= -->
       ...
       %>
         <tr id="trItem" class="DataTable<%if(sOldItem.equals("1")){%>2<%}%>">
            ...
            <td class="DataTable1" id="tdIpSizNm<%=i%>" nowrap><%=sUpc%></td>
          </tr>
          <script>if(SelProdType == null){ SelProdType = "<%=sProdType%>"; SelProdTypeId = "<%=sProdTypeId%>";  }</script>
       <%}%>
     </table>
<!-- ======================================================================= -->
