<%@ page import="payrollreports.PsSavSchedEnt, java.util.*"%>
<% // Get query string parameters
   ...
   {
     response.sendRedirect("SignOn1.jsp?TARGET=PsSavSchedEnt.jsp&APPL=" + sAppl + "&" + request.getQueryString());
   }
   else
   {
     ...
     PsSavSchedEnt svScd = new PsSavSchedEnt();

     //System.out.println("JSP. ==> Group:" + sGrp + "  Emp: " + sEmpNum + " " + sEmpName
     // + "Date: " + sWeekDay[0] + " ACTION: " + sAction + " Key: " + sKey + " ActRange:" + sActRng);
     ...
     for(int i=0; i<sWeekDay.length; i++)
     {
        if (sWeekDay[i] != null)
        {
         System.out.println(sStore + "|" + sWeekDay[i] + "|" + sGrp + "|" + sEmpNum
           + "|" + sEmpName + "|" + sBegTime[i] + "|" + sEndTime[i] + "|" + sAction
           + "|" + sHrsType[i] + "|" + sActRng + "|" + sWeekEnd + "|" + sNewGrp + "|" + sKey
           + "|" + sUserId);
         svScd.savSingleDay(sStore, sWeekDay[i], sGrp, sEmpNum,
              sEmpName, sBegTime[i], sEndTime[i], sAction, sHrsType[i],
              sActRng, sWeekEnd, sNewGrp, sKey, sUserId);
         ...
         if(ierr > 0)
         {
            ...
            serr = svScd.getError();
            for(int k=0, j = iNumOfErr; k < ierr; k++, j++)
            {
               sError[j] = serr[k];
            }
            iNumOfErr += ierr;
