<%@ page import="payrollreports.PsWkSched,  rciutility.SetStrEmp, java.util.*"%>
<%
  ...
  boolean bChange = false;
   ...
   {
     response.sendRedirect("SignOn1.jsp?TARGET=PsWkSched.jsp&" + request.getQueryString());
   }
   else
   {
     ...
     bChange = session.getAttribute("USER").toString().equals("vrozen")
    		 || session.getAttribute("USER").toString().equals("srutherfor");

     vStr =  (Vector) session.getAttribute("STRLST");
     sStrAlwLst = new String[ vStr.size()];
     Iterator iter = vStr.iterator();

     bSelStrAllowed = sStrAllowed.startsWith("ALL");

     while (iter.hasNext())
     {
       sStrAlwLst[iStrAlwLst] = (String) iter.next();
       if(!bSelStrAllowed && sStrAlwLst[iStrAlwLst].equals(sStore)){ bSelStrAllowed = true; }
       iStrAlwLst++;
     }
     if(!bSelStrAllowed){ response.sendRedirect("/"); }
   }
  // -------------- End Security -----------------
    PsWkSched sWkSch = new PsWkSched(sStore, sWeekEnd);
    ...
    String sSecNameJsa = sWkSch.getSecNameJsa();
    String [] sGrpLst = sWkSch.getGrpLst();
    String [] sGrpName = sWkSch.getGrpName();
    String [] sSecTot = sWkSch.getSecTot();
    String [] sGrpSecLst = sWkSch.getGrpSecLst();
    String [] sGrpSls = sWkSch.getGrpSls();
    String [] sGrpSecSls = sWkSch.getGrpSecSls();
    ...
    String [][] sGrpAvgHrs = sWkSch.getGrpAvgHrs();
   ...
   // Hours/Minutes array
   String [] sHrsTxt = new String[]{"06","07","08","09","10","11","12",
                                 "01","02","03","04","05","06",
                                 "07","08","09","10","11","12"};
   String [] sHrs = new String[19];
   String [] sMin = new String[]{"00", "15", "30", "45"};
   for(int i=6; i<25;i++){
     sHrs[i-6] = Integer.toString(i);
     if (sHrs[i-6].length()== 1) {
        sHrs[i-6] = "0" + sHrs[i-6];
     }
   }
   ...
   {
      for(int j=iAll; j<iNumGrpEmp[i]; j++, iAll++){ sAllEmp[j] = sGrpSch[i][j][0][0];}
   }

   // Use name of base weeks instead of date
   String sWeekName = sWeekEnd;
   if(sWeekEnd.indexOf("2099") >= 0 )
   {
   ...
   String sSecByHrs = sWkSch.getEmpByHrs();

   sWkSch.setActSls();
   String [] sActSls = sWkSch.getActSls();
   String [] sLyActSls = sWkSch.getLyActSls();
   ...
   boolean bAlwBdgAuth = session.getAttribute("PRLAB") != null;
...
<style>
        ...
        th.DataTable3 { background:#FFCC99; border-top: darkred solid 1px;border-left: darkred solid 1px;
        ...
        tr.SecHdr1 { background:cornsilk; }
        tr.SecHdr10 { background: #ccffcc; }

        tr.SecHdr2 { background:Linen; }
        tr.SecHdr20 { background: #ccccff; }
        ...
        td.DataTable10 { color:brown;  <%if(bChange){%>cursor: hand;<%}%> border-top: darkred solid 1px; border-bottom: darkred solid 1px; padding-left:30px; padding-top:0px; padding-bottom:0px; border-right: darkred solid 1px; text-align:left; font-family:Arial; font-size:10px }
        td.DataTable11 { color:red; <%if(bChange){%>cursor: hand;<%}%> border-top: double darkred; border-bottom: darkred solid 1px;  border-right: darkred solid 1px;padding-top:3px; padding-bottom:3px; text-align:left; font-family:Arial; font-size:11px; font-weight:bold}
        ...
        td.DataTable13 { color:red; border-bottom: darkred solid 1px; border-top: double darkred; padding-top:3px; padding-bottom:3px; border-right: darkred solid 1px; text-align:center; font-family:Arial; font-size:10px }
        td.DataTable14 { color:red; border-bottom: darkred solid 1px; border-top: double darkred; padding-top:3px; padding-bottom:3px; text-align:right; font-family:Arial; font-size:10px }
        ...
        td.DataTable17 { border-top: darkred solid 1px; border-bottom: darkred solid 1px; padding-top:0px; padding-bottom:0px; border-right: darkred solid 1px; text-align:left; font-family:Arial; font-size:10px }
        ...
        td.Grid  { background: #016aab; color:white; text-align:center; font-family:Arial; font-size:11px;
                   padding-left:3px; padding-right:3px; font-weight:bolder}
        tr.Grid1 { text-align:center; font-family:Arial; font-size:10px;}
        td.Grid2  { background: #016aab; color:white; text-align:right; font-family:Arial; font-size:11px; font-weight:bolder}
        ...
        tr.Banner{ text-align:center; font-family:Arial; font-size:16px; }
...
<%for(int i=0; i < sEmpNum.length; i++){%>EmpNum[<%=i%>] = "<%=sEmpNum[i]%>";  EmpName[<%=i%>] = "<%=sEmpName[i]%>"; EmpEMail[<%=i%>] = "<%=sEmpEMail[i]%>"; DptName[<%=i%>] = "<%=sDptName[i]%>";   DptType[<%=i%>] = "<%=sDptType[i]%>";<%}%>
...
var SecByHrs = [ <%=sSecByHrs%> ];
var SecName = [<%=sSecNameJsa%>];
...
function bodyLoad()
{
  ...
  //chkClinics(); ===> no cliniks pop ups

  //addButton();
  <%if(bChange){%>addWkBudget();<%}%>
  rtvBdgVsSched();
  <%if(!sUser.equals("vrozen") && !sUser.equals("snixon") && !sUser.equals("mparker")){%>
  hideAvgHrs();
  <%}%>
...
function getBanner()
{	 
	 var url = "PrSchedBanner.jsp?str=" + CurStore;
	 var xmlhttp;
	 ...
	 xmlhttp.onreadystatechange=function()  
	 {
	    ...
	    {
	  	  var banner = xmlhttp.responseText;	  	  
	  	  var obj = jQuery.parseJSON(banner);
	  	  document.all.tdBanner.style.display = "none";
	  	  
	  	$.each(obj.banner, function(i,item){
	          var text = item.text;
	          var size = item.size;          
	          var font = item.font;
	          ...
	          document.all.tdBanner.innerHTML = text;
	          document.all.tdBanner.style.fontSize = size;
	          document.all.tdBanner.style.fontFamily = font;
	          document.all.tdBanner.style.color = color;
	          document.all.tdBanner.style.background = background;
	          document.all.tdBanner.style.display = "block";
		});
   ...
   //document.all.tblBanner.style.display = "none";
}
//==============================================================================
// hide average hours
//==============================================================================
function hideAvgHrs()
{
   var cell = document.all.cellAvgHrs;
...
function rtvBdgVsSched()
{
   ...
   //alert(url)
   //window.location = url;
   if(isIE || isSafari){ window.frame3.location.href = url; }
...
{
  var html = "<table cellPadding='0' cellSpacing='0' width='100%' style='background: LemonChiffon;'>"
      html += "<tr align='center'>"
         ...
         + "</td></tr>"
         + "<tr>"
         + "<td colspan=2>" + popBdgVsSchedVarPanel(BdgHrs, SchHrs, HrsVar, GrpName, GrpBdgHrs, GrpSchHrs, GrpHrsVar) + "</td></tr>"
  ...
  if(isIE && ua.indexOf("MSIE 7.0") >= 0){ document.all.dvBdgVsSchedVar.style.width = "250"; }
  else { document.all.dvBdgVsSchedVar.style.width = "auto"; }
  ...
  document.all.dvBdgVsSchedVar.style.left=10;
  document.all.dvBdgVsSchedVar.style.top=95;  
...
function addWkBudget()
{
   var html = "<table border=1 style='font-size:10px' cellPadding='0' cellSpacing='0'>"
       ...
       //+ "</tr>"
       + "<tr>"
         + "<td nowrap><a a target='_blank' href='PsWkSched.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sBaseWkend%>'>Base Schedule<a></td>"
       + "</tr>"

   <%if(bAlwBdgAuth){%>
      html += "<tr>"
              + "<td nowrap><a target='_blank' href='BfdgSchActWk.jsp?Store=<%=sStore%>&StrName=<%=sThisStrName%>&Wkend=<%=sWeekEnd%>'>Allowable Budget<a></td>"
            + "</tr>"
   <%}%>

   <%if(bChange){%>
      html += "<tr>"
          + "<td nowrap><a target='_blank' href='PsEmpNum.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[0]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[0]%>&SELSEC=SELL'>Sales Coverage</a>"
       + "</tr>"
   <%}%>
   ...
   document.all.CmdButton.style.pixelTop=380
...
{
 var schtyp = null;
 if (type == "Hours") {  schtyp = 'HRS'; }
 else { schtyp = 'TIM'; }
 
 var loc = "PsWkSched.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>"
         + "&SCHTYP=" + schtyp
         + "&SHWGOAL=<%=sShwGoal%>";
...
function chgSelDate()
{
 ...
 var loc = "PsWkSched.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>"
         + "&MONBEG=" + mon
         + "&WEEKEND=" + newWeekEnd
...
//==============================================================================
function ShowMenu(obj, igrp, iemp, iday, goal, tmOff, key)
{
 ...
 var MenuSub = " ";
 var MenuAdd = " ";
 var MenuSubst = " "; // substitute employee
 var MenuNew = " ";
 var MenuVac = " ";
 var MenuHol = " ";
 var MenuOff = " ";
 ...
 if(iday != null)
 {
   ...
   if(isCopyHrsAllowed(obj, emp, grp, day)){
       MenuCpyDly = "<tr><td colspan='2' class='Menu' align='center' "
              + "onclick='copyDlyHrs(&#34;"
              + emp + "&#34;, &#34;" + grp + "&#34;, &#34;" + day + "&#34;, &#34;" +  key + "&#34;); hideMenu();'>Copy Hours"
              + "</td></tr>"
   }
   ...
   if(isPasteHrsAllowed(obj, emp, grp, day)){
       MenuPasteDly = "<tr><td colspan='2' class='Menu' align='center' "
              + "onclick='pasteDlyHrs(&#34;"
              + emp + "&#34;, &#34;" + grp + "&#34;, &#34;" + day + "&#34;, &#34;" +  key + "&#34;); hideMenu();'>Paste Hours"
              + "</td></tr>"
   }
 ...
 // customize menu options for selected employee
   ...
   MenuSubst = "<tr><td class='Menu' colspan='2' align='center' "
           + "onclick='addNew(&#34;" + grp + "&#34;, &#34;" + grpname + "&#34;, "
           + "&#34;" + subgrp + "&#34;, "
           + "&#34;" + emp + "&#34;,&#34;Y&#34;);hideMenu();'>Substitute"
           + "</td></tr>";
   ...
   MenuPrt = "<tr><td class='Menu' colspan='2' align='center' "
           + "onclick='prtEmp(&#34;" + emp + "&#34;); hideMenu();'>Print</td></tr>"
   MenuEMail = "<tr><td class='Menu' colspan='2' align='center' "
           + "onclick='sendSchedByEmail(&#34;" + emp + "&#34;); hideMenu();'>E-Mail</td></tr>"
 }
 // customize menu options for selected group
   ...
   MenuAdd = "<tr><td class='Menu' colspan='2' align='center' "
           ...
           + ");hideMenu();'>Add"
           + "</td></tr>";
   if (GrpLst[igrp] == "CASHR" || GrpLst[igrp] == "TRAIN")
   {
 ...
 MenuHtml = "<table width='100%' cellPadding='0' cellSpacing='0'>"
   ...
   + "</td></tr>"
   + MenuGrp + MenuSub + MenuGoal + MenuAdd + MenuSubst + MenuNew + MenuCpy + MenuVac + MenuHol + MenuOff
   + MenuMov + MenuDel + MenuAvl + MenuCpyDly + MenuPasteDly + MenuPrt + MenuEMail
...
// save copied Hours
//==============================================================================
function copyDlyHrs(emp, grp, day, key)
{
   empDayHrsCopy = new EmpDayHrs(emp, grp, day, key);
}
...
//==============================================================================
function pasteDlyHrs(emp, grp, day, key)
{
    // change action string
    var url = "PsSavSchedEntCopy.jsp?STORE=" + CurStore
          + "&STRNAME=" + CurStrName
          + "&MONBEG=" + Month
          + "&WEEKEND=" + WeekEnd
...
{
   var url="PsEmpEvtSave.jsp?Str=" + CurStore
...
//==============================================================================
function addNew(grp, grpname, subgrp, emp, substitute){
 ...
 var oldemp = null;
 if(substitute != null){ oldemp = emp; }

 var newEmpHtml = "<table cellPadding='0' cellSpacing='0'>"

    if(emp==null || substitute != null){
    ...
    if(substitute == null)
    {
      newEmpHtml += "&nbsp;&nbsp;<button class='small' name='getEmp' onclick='getEmpDaySel(" + param + ", &#34;&#34;)'>Continue</button>"
    }
    else
    {
      newEmpHtml += "&nbsp;&nbsp;<button class='small' name='getEmp' onclick='sbmSubstitute(" + param + ", &#34;&#34;)'>Continue</button>"
    }

    newEmpHtml += "&nbsp;&nbsp;<button class='small' name='chkDays' onclick='chgAlldays(true)'>All Days</button>"
    + "&nbsp;&nbsp;<button class='small' name='chkDays' onclick='chgAlldays(false)'>Reset</button>"
    + "&nbsp;&nbsp;<button class='small' name='close' onclick='hidetip2()'>Close</button><br>&nbsp;</td></tr>"
    + "</table>"

    document.all.tooltip2.innerHTML=newEmpHtml
    document.all.tooltip2.style.pixelLeft=300
    document.all.tooltip2.style.pixelTop=document.documentElement.scrollTop+40
    document.all.tooltip2.style.visibility="visible"

    loadSelEmp(grp, emp, substitute);
    if(substitute == null  && emp!=null && emp.substring(5, 13) != "New Hire") { checkAvail(emp); }
...
function selectWeekEnd(type, emp, grp, range){
 var weHtml = "<table cellPadding='0' cellSpacing='0' style='background: LemonChiffon'>"
    + "<tr align='right'>"
    + "<td nowrap>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
...
if(type!='DAY')
{
  weHtml += "<br><font size='-1' color='red'>**This copy feature will NOT overwrite an employee's already existing scheduled day for this future week."
   ...
   weHtml +="</font></td></tr></table>"
	...
	if(isIE && ua.indexOf("MSIE 7.0") >= 0){ document.all.tooltip2.style.width = "250";}
	else { document.all.tooltip2.style.width = "auto";}
   
    document.all.tooltip2.innerHTML=weHtml
    document.all.tooltip2.style.left=getLeftScreenPos() + 500; 
    document.all.tooltip2.style.top=getTopScreenPos()+20
    document.all.tooltip2.style.visibility="visible"
...
// get employee and days
//==============================================================================
function getEmpDaySel(grp, grpname, emp, day, key)
{
 ...
 if(isEmpDaySelValid()) {
   ...
   for(i=0;i<7;i++)
   {
     ...
     {
        SelDays[i]=false;
        showTimeEntry(grp, grpname, WkDate[i], WkDays[i], key);
        break;
     }
   }
 }
...
// validate Employee Day Selection
//==============================================================================
function sbmSubstitute(grp, grpname, oldemp, day, key)
{
   selectedEmployee = document.all.SelEmp.options[document.all.SelEmp.selectedIndex].value
   if(EmpName.length > document.all.SelEmp.selectedIndex)
   {
   ...
   if(isEmpDaySelValid())
   {
      var url = "PsSavSchedSubstitute.jsp?STORE=" + CurStore
          + "&STRNAME=" + CurStrName
          + "&MONBEG=" + Month
          + "&WEEKEND=" + WeekEnd
          + "&CPYEMP=" + oldemp
          + "&CPYGRP=" + grp
          + "&TOEMP=" + selectedEmployee
...
//==============================================================================
function savSingleDay(day, skip, TimeType, key){
  var found = false
  var apply = false;
  if (!skip)
  {
...
//==============================================================================
function showTimeEntry(grp, grpname, day, dayofWeek, key){
 ...
 var entryTimeHtml = "<form name='HRSENTRY' method='GET' >"
    ...
    + "<td class='EntTbl' rowspan='2'><button class='small' name='ResetEntry' type='button' DISABLED onclick='resetHrs();'>Reset&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button></td>"
     ...
     + "<tr >"
      ...
      + " onclick='if (Validate(document.HRSENTRY, &#34;" + day+"&#34;, &#34;" + key + "&#34;)){ savSingleDay(&#34;"
      + day +"&#34;,false, &#34;REG&#34;, &#34;" + key + "&#34;);}'>Regular&nbsp;&nbsp;</button>"
      + "</td>"
      ...
      if(selectedTimOffType != "RQOFF" && selectedEmployee.substring(5, 13) != "New Hire")
      {
        entryTimeHtml += "<tr>"
          ...
          + "</td>"
          + "<td class='EntTbl' id='VAC8HRS' colspan='19'>"
          ...
          + "</td>"
          + "<td class='EntTbl' id='HOL8HRS' colspan='19'>"
...
function dspEmpbyHr(obj, day, dayOfWeek)
{
  ...
  {
    empHrHtml += "<th class='DataTable3'>" + SecName[i] + "</td>"
  }
  empHrHtml += "<th class='EntTbl'>&nbsp;</th>";
  empHrHtml += "<th class='DataTable3'>Total</th></tr>";

  var cnt = 0;
  for(i=0; i < 19; i++)
  {
      empHrHtml += "<tr>" + "<td class='EntTbl'>" + hrs[i] + "</td>"
      for(var j=0; j < SecByHrs.length; j++)
      {
         if(j==SecByHrs.length - 1){empHrHtml += "<th class='EntTbl'>&nbsp;</th>";}
         empHrHtml += "<td class='EntTbl'>" + SecByHrs[j][Day][i] + "</td>"
      }
      empHrHtml += "</tr>"
   }
...
function dspGrpbyHr(sec, secnm)
{
  ...
  for(i=0; i < 19; i++)
  {
      ...
      for(j=0; j < 7; j++)
      {
         ...
         for(var k=0; k < SecByHrs.length; k++)
         {
             if(sec == k) empHrHtml += "<td class='EntTbl'>" + SecByHrs[k][j][i] + "</td>"
         }
     }
     empHrHtml +="</tr>"
   }
  ...
  document.all.tooltip2.innerHTML=empHrHtml
  document.all.tooltip2.style.pixelLeft=curLeft + 200
  document.all.tooltip2.style.pixelTop=document.documentElement.scrollTop+10
  document.all.tooltip2.style.visibility="visible"
}
//==============================================================================
...
//==============================================================================
function  loadSelEmp(grp, emp, substitute){
...
function resetHrs(){
 if(cells!=null){
   for(var i=0; i < cells.length; i++){
     if (cells.item(i).id >= "0600" && cells.item(i).id <= "2400")
...
function Validate(docFrm, day, key){
...
function submit(){
  ...
  // change action string
  SbmString = "PsSavSchedEnt.jsp?STORE=" + CurStore
                + "&STRNAME=" + CurStrName
                + "&MONBEG=" + Month
                + "&WEEKEND=" + WeekEnd
                + "&GRP=" + selectedGrp
  ...
  for (i=0;i<=savArg;i++)
  {
     SbmString = SbmString + "&WKDATE" + i + "=" + savDate[i] + "&Key=" + savKey[i]
              + "&BEGTIME" + i + "=" + savBegTim[i]
              + "&ENDTIME" + i + "=" + savEndTim[i]
              + "&HRSTYP" + i + "=" + selectedHrsType[i];
    if(savKey[i] != "") { action = "OVR"; }
  }

  SbmString += "&ACTION=" + action;

  //alert(SbmString)
  if (Allow=="YES") { window.location.href = SbmString; }
  else alert("This week has the D status. Changes is not allowed");
}

//==============================================================================
// delete employee schedule
//==============================================================================
function dltEntry(emp, grp, day, range, key){
...
//==============================================================================
function genReqOff(emp, grp, day, key){
  var action = "ADD";
  if(key !="") { action = "OVR";}
  SbmString = "PsSavSchedEnt.jsp?STORE=" + CurStore
                + "&STRNAME=" + CurStrName
...
function submitEvt()
{
  hidetip2();
  // change action string
  SbmString = "SavEvtEnt01.jsp?STORE=" + CurStore
                + "&STRNAME=" + CurStrName
                + "&MONBEG=" + Month
                + "&WEEKEND=" + WeekEnd
                + "&EVENT=" + selectedEvent
...
//==============================================================================
function EmpDayHrs(emp, grp, day, key)
{
   this.emp = emp;
   this.grp = grp;
   this.day = day;
...
//==============================================================================
if(!EmpDayHrs.prototype.getEmpName){ EmpDayHrs.prototype.getEmpName = function(){ return this.emp; } }
if(!EmpDayHrs.prototype.getGroup){EmpDayHrs.prototype.getGroup = function() { return this.grp; } }
if(!EmpDayHrs.prototype.getDay) { EmpDayHrs.prototype.getDay = function(){ return this.day; } }
if(!EmpDayHrs.prototype.getKey){EmpDayHrs.prototype.getKey = function(){ return this.key; } }
//==============================================================================
...
//==============================================================================
function prtEmp(empName)
{
   var html = prtIndividSched(empName, true);

   var MyWindowName = "My_Schedule";
   var MyWindowOptions =
   ...
   var win = window.open("", MyWindowName, MyWindowOptions);
...
//==============================================================================
function sendSchedByEmail(empName)
{
   ...
   document.all.body.value = prtIndividSched(empName, false);
   document.all.emp.value = empName.substring(0, 4);
   popEMailToAddr(empName);
}
//==============================================================================
...
function sbmSchedByEmail()
{
   ...
   window.frame4.document.appendChild(nwelem);

   window.frame4.document.all.from.value = "gm<%=sStore%>@retailconcepts.cc";
   window.frame4.document.all.subject.value=frmeml.subject.value;
   window.frame4.document.all.to.value=frmeml.to.value;
   window.frame4.document.all.cc.value=frmeml.cc.value;
   window.frame4.document.all.bcc.value=frmeml.bcc.value;
   window.frame4.document.all.body.value=frmeml.body.value;

   window.frame4.document.frmSbmEml.submit();
   hideEMail();
...
//==============================================================================
// populate Individule schedule body
//==============================================================================
function prtIndividSched(empName, print)
{
   var tb = document.all.tbSched;
   var allrows = tb.getElementsByTagName("tr");
   var trEmp = new Array();
   var cell = null;
   var secrow = null;
   var grprow = null;

   for(var i=0, j=0; i < allrows.length; i++)
   {
      cell = allrows[i].getElementsByTagName("td");

      // search section name
      if(cell != null && cell[0] != null && cell[0].id.indexOf("SECTION") >= 0)
      {
        secrow = allrows[i];
        var seccell = secrow.getElementsByTagName("td");
        for(var k=1; k < seccell.length; k++)
        {
           seccell[k].innerHTML = "&nbsp";
        }
      }

      // search group name
      if(cell != null && cell[0] != null && cell[0].id.indexOf("_GP") >= 0)
      {
        grprow = allrows[i];
        var grpcell = grprow.getElementsByTagName("td");
        for(var k=1; k < grpcell.length; k++)
        {
           grpcell[k].innerHTML = "&nbsp";
        }
      }

      //  search employee name
      if(cell != null && cell[0] != null
         && cell[0].id.indexOf("_EMP") >= 0 && cell[0].innerHTML.indexOf(empName) >= 0)
      {
        trEmp[j++] = secrow;
        trEmp[j++] = grprow;
        trEmp[j++] = allrows[i];
   ...
   var html = "<style> tr.DataTable3 { background: #ccffcc; font-size:10px }"
     + " td.DataTable { background:#e7e7e7; cursor: hand; padding-top:3px; padding-bottom:3px; text-align:left; font-family:Arial; font-size:10px }"
     + " td.DataTable4 { background:#e7e7e7; padding-top:3px; padding-bottom:3px; text-align:center; font-family:Arial; font-size:10px }"
     + " td.DataTable10 { color:brown; padding-left:30px; padding-top:0px; padding-bottom:0px; text-align:left; font-family:Arial; font-size:10px }"
     + " td.DataTable11 { color:red; padding-top:3px; padding-bottom:3px; text-align:left; font-family:Arial; font-size:11px; font-weight:bold}"
     + "</style><br>"
     + "<BODY "

     if (print){ html +="onload='window.print();window.close();'>" }

     html += "<div width=500><p align=center><b>Store:<%=sStore + " - " + sThisStrName%>"
     + "<br>Week Ending:&nbsp;<%=sWeekName%></b>"
     + "<table border=1 cellPadding='0' cellSpacing='0'>"
     + "<tr><th>Employee</th>"
     + "<th colspan=2>Title</th>"
     + "<th>Mon</th><th>Tue</th><th>Wed</th><th>Thu</th><th>Fri</th><th>Sat</th><th>Sun</th><th>Total</th></tr>";

   for(var i=0; i < trEmp.length; i++)
   {
       cell = trEmp[i].getElementsByTagName("td");
       html += "<tr>"
       for(var j=0; j < cell.length; j++)
       {
          html += cell[j].outerHTML;
       }
       html += "</tr>"
   }
...
<!-------------------------------------------------------------------->
  ...
  <div id="dvEMail" class="dvClinic"></div>

<!-------------------------------------------------------------------->
  <table border="0" width="100%" cellPadding="0" cellSpacing="0" id="tblBanner">
     <tr class="Banner" id="trBanner">
        <td id="tdBanner"></td>
     </tr>
  </table>
  <table border="0" width="100%" height="100%" cellPadding="0" cellSpacing="0">
     <!-- >  tr class="Banner" colspan="3" id="trBanner">
      <td id="tdBanner">Banner</td>
     </tr -->
     <tr bgColor="moccasin">
      <td ALIGN="center" VALIGN="TOP">&nbsp;</td>
      <td ALIGN="center" VALIGN="TOP"><b>Weekly Schedule Summary(New Schedule)</b></td>
      <td ALIGN="right" VALIGN="TOP">&nbsp;</td> 
     </tr>
     <tr bgColor="moccasin">
        <td ALIGN="center" VALIGN="TOP">&nbsp;</td>
        <td ALIGN="center" VALIGN="TOP" width="33%"><b>&nbsp;Store:&nbsp;<%=sStore + " - " + sThisStrName.trim()%></b></td>        
     ...
     <tr bgColor="moccasin">
        ...
        <%}%>
         This page &nbsp;&nbsp;&nbsp;
         <a href="PsWkSchedPrt.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&WEEKEND=<%=sWeekEnd%>" class="blue">Print By Employee Name</a>
      ...
      <table class="DataTable" cellPadding="0" cellSpacing="0" id="tbSched">
         <tr>
           ...
           <th class="DataTable" rowspan="4" id="thTotal">Total</th>
           <th class="DataTable" rowspan="8" id="cellAvgHrs">&nbsp;Avg&nbsp;<br>&nbsp;Hrs&nbsp;</th>
         </tr>
         <tr>
           <%for(int i=0; i < 7; i++){%>
             <th class="DataTable" >
              <%if(bChange){%>
               <a href="PsDaySched.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[i]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[i]%>">
                <%=sShort[i]%></a>
              <%} else {%><%=sShort[i]%><%}%>
         ...
         <tr>
           <th class="DataTable1" colspan="3">
            <%if(bChange){%>
                <a href="PsEmpNumStr.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[0]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[0]%>&SELSEC=ALL">
                    Store Coverage</a>
            <%} else{%>&nbsp;<%}%>
           </th>
           <%for(int i=0; i < 7; i++){%>
         ...
         <tr>
           <th class="DataTable1" colspan="3">Floor Leader Agenda</th>
           <%for(int i=0; i < 7; i++){%>
             <th class="DataTable2" id="EBH<%=i%>">
                <a href="StrDlyToDoList1.jsp?Store=<%=sStore%>&Date=<%=sWkDate[i]%>" target="_blank"><%=sWkDays[i]%></a></th>
           <%}%>
         </tr>
     ...
     <!-- --------------- Actual Sales --------------------- -->
         ...
         <tr>
           <th class="DataTable1" colspan="3">LY Actual Sales</th>
           <%for(int i=0; i < 8; i++){%>
             <th class="DataTable" ><%=sLyActSls[i]%></th>
           <%}%>
         </tr>
         ...
         <tr>
            ...
            <%for(int l=0; l< 7; l++){%>
                  <td class="DataTable16" >$<%=sSlsGoalHr[2][l]%></td>
            <%}%>
            <td class="DataTable16">$<%=sSlsGoalHr[2][7]%></td>
            <td class="DataTable16">&nbsp;</td>
         </tr>
   <!-- ------------------------ Sections Header ------------------------------------------ -->
   <%int iSec = 0;
     int iSecGrp = 0;
     for(int i=0; i < iNumOfGrp; i++){%>
       <%if(iSecGrp == 0){%>
         <tr class="SecHdr1<%if(sSecLst[iSec].equals("FLRLD")){%>0<%}%>">
           ...
           <td class="DataTable14">
             <%if(sSecLst[iSec].equals("MNGR")){%><%if(bChange){%><a href="javascript:dspGrpbyHr('<%=iSec%>', '<%=sSecName[iSec]%>')">Cov</a><%} else {%>&nbsp;<%}%><%}
               else {%><%if(bChange && !sSecLst[iSec].equals("FLRLD")){%><a href="PsEmpNum.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[0]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[0]%>&SELSEC=<%=sSecLst[iSec]%>">Cov</a><%} else {%>&nbsp;<%}%><%}%>
           </td>
           </td>
             ...
             <td class="DataTable13" >&nbsp;</td>
         </tr>
       <%}%>
       <!----------------------- Group Header -------------------------------------------------->
         <tr class="SecHdr2<%if(sSecLst[iSec].equals("FLRLD")){%>0<%}%>">
           ...
           <td class="DataTable17" colspan="9">&nbsp;
              <%if(bChange && !sSecLst[iSec].equals("FLRLD")){%>
                <a href="PsEmpNum.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[0]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[0]%>&SELGRP=<%=sGrpLst[i]%>">
        ...
        <%for(int j=0; j < iNumGrpEmp[i]; j++){%>
           <tr id="trDtl">
            ...
            <%for(int k=0; k < 7; k++)
             ...
             %>
                <td class="DataTable" <%if(bChange){%> id="tdHrs"<%}%> nowrap>

                   <%for(int l=0; l < sGrpSch[i][j][k].length; l++)
                   {
                      String skValue = sGrpSch[i][j][k][l];
                      String skClass = "";
                      if(sGrpHTyp[i][j][k][l].equals("VAC")){ skClass = "#afdcec;"; skValue = "Vacation"; }
                      else if(sGrpHTyp[i][j][k][l].equals("HOL")){ skClass = "#99c68e;"; skValue = "Holiday"; }
                      else if(sGrpHTyp[i][j][k][l].equals("OFF")) { skClass = "gold"; skValue = "Request Off"; }
                      else { skClass = "#e7e7e7;";}
                    %>
                      <div style="background:<%=skClass%>" <%if(bChange){%>onclick="ShowMenu(this, '<%=i%>', '<%=j%>', '<%=k%>', '0', '<%=sGrpTimOff[i][j]%>', '<%=sGrpKey[i][j][k][l]%>')"<%}%>><%=skValue%></div>
                 <%}%>
                    <%if(sGrpSls[i].equals("1") && sShwGoal.equals("S") && sSlsGoal[i][j] != null){%>
                       <%=sSlsGoal[i][j][k]%><%}%>
                </td>
                </td>
              <%}%>
              <td class="DataTable4"><font color="<%=sWkSch.chkOvrTim(sGrpEmp[i][j])[0]%>"><%=sGrpEmpTot[i][j]%><%=sWkSch.chkOvrTim(sGrpEmp[i][j])[1]%></font></td>
              <td class="DataTable4" id="cellAvgHrs"><%if(!sGrpAvgHrs[i][j].equals(".0")){%><%=sGrpAvgHrs[i][j]%><%} else{%>&nbsp;<%}%></td)
           </tr>
       <%}%>
      ...
      <!-- --------------- Store Weekly Events ---------------- -->
      <tr>
       <td class="DataTable11" id="EVTGP" colspan="12"
...
<!------------- end of data table ------------------------>
       ...
       <p align="center" >
       <button name="CHGSCH" type="button" onclick="chgSch('<%if(sSchTyp.equals("TIM")){%>Hours<%} else {%>Schedule<%}%>')" >
         <%if(sSchTyp.equals("TIM")){%>Hours<%} else {%>Schedule<%}%>
       </button>&nbsp;&nbsp;
       <button name="SHWGOAL" type="button" onclick="dspGoal('<%if(sShwGoal.equals("S")){%>H<%} else {%>S<%}%>')">
          <%if(sShwGoal.equals("S")){%>Hide Sales Goal<%} else {%>Sales Goal<%}%>
       </button>

       <%if(bChange){%>
          <br><font size="-1">Click here to see the Monthly</font>
             <a href="PsMonBdg.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>">
