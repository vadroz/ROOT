...
<%
   ...
   if(sLevel==null) { sLevel = "I"; }

   SlsVelocityGrp slsVel = null;

   if(sSelCls == null)
   {
       slsVel = new SlsVelocityGrp(sGroup, sSelStr, sFrWeek, sToWeek, sLevel, sSortBy);
   }
   else
   {
       slsVel = new SlsVelocityGrp(sSelCls, sSelVen, sSelSty, sSelStr
              , sFrWeek, sToWeek);
   }
...
{
   var url = "SlsVelocityGrp.jsp?Cls=" + cls
     + "&Ven=" + ven
     + "&Sty=" + sty
     + "&FrWeek=" + FrWeek
     + "&ToWeek=" + ToWeek
   ...
   window.open(url, "Item_Group_Sellers_Details")
...
function switchOnhand(type)
{
   ...
   window.status = "Reset Invnetory Line... "
   ...
   for(var i=0; i < spincl.length; i++)
   {
      ...
      if(i % 2000 == 0){ window.status = "Reset Invnetory... " + i; }
   }
   window.status = "Done"
}
</SCRIPT>
...
<!-------------------------------------------------------------------->
      ...
      <table class="DataTable" cellPadding="0" cellSpacing="0">
        ...
        <tr style="z-index: 60; position: relative; left: -1; top: expression(this.offsetParent.scrollTop-2);">
          ...
          <th class="DataTable" colspan=30>&nbsp;</th>
        </tr>
        ...
        <tr style="z-index: 60; position: relative; left: -1; top: expression(this.offsetParent.scrollTop-2);">
          ...
          <th class="DataTable" colspan="3">Total Units</th>
        ...
        <tr style="z-index: 60; position: relative; left: -1; top: expression(this.offsetParent.scrollTop-2);">
          ...
          <th class="DataTable" >Open<br>PO</th>
          <th class="DataTable" >BSR<br>Var</th>
...
<!------------------------------- Data Detail --------------------------------->
           ...
           <%for(int i=0; i < iNumOfItm; i++) {
              ...
              String [] sBsrVar = slsVel.getBsrVar();
              String sTotBsrVar = slsVel.getTotBsrVar();
              String [] sBsrVarEx = slsVel.getBsrVarEx();
              String sTotBsrVarEx = slsVel.getTotBsrVarEx();
           %>

           <%if(sSelCls==null){%>
                <%if(bOdd) { sClass = "DataTable";}
           ...
           <%}%>
              ...
              <tr class=<%=sClass%> id="tr<%=sCls+sVen+sSty%>0">
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sDiv%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sDpt%></td>
                <td class="DataTable1" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>" nowrap>
                    <%if(sLevel.equals("S")){%>
                ...
                <td class="DataTable1" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>" nowrap><%if(bCvs){%><%=sDesc%><%}%>&nbsp;</td>
                <td class="DataTable1" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>" nowrap><%if(bCvs){%><%=sVenName%><%}%>&nbsp;</td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sChnRet%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sTotSlsUnit%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sTotSlsRet%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sTotOnhand%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>"><%=sTotTrans%></td>
                <td class="DataTable" rowspan="<%if(!bCvs){%>2<%} else {%>3<%}%>">
                      <span id="spIncl" <%if(!sTotBsrVar.equals("0")){%>style="background:red;"<%}%>><%if(bCvs && !sTotBsrVar.equals("0")){%><%=sTotBsrVar%>%<%} else {%>&nbsp;<%}%></span>
                      <span id="spExcl" <%if(!sTotBsrVarEx.equals("0")){%>style="background:red;"<%}%>style="display:none;"><%if(bCvs){%><%=sTotBsrVarEx%>%<%} else {%>&nbsp;<%}%></span>
                      <span id="spOnly" style="display:none;"></span>
                </td>
              ...
              <%if(bCvs){%>
                 <tr class=<%=sClass%> id="tr<%=sCls+sVen+sSty%>1">
                   <td class="DataTable3">Var</td>
                      <%for(int j=0; j < iNumOfStr; j++) {%>
                         <td class="DataTable">
                           <span id="spIncl" <%if(!sBsrVar[j].equals("0")){%>style="background:red;"<%}%>><%if(!sBsrVar[j].equals("0")){%><%=sBsrVar[j]%>%<%} else {%>&nbsp;<%}%></span>
                           <span id="spExcl" <%if(!sBsrVarEx[j].equals("0")){%>style="background:red;"<%}%> style="display:none;" ><%if(!sBsrVarEx[j].equals("0")){%><%=sBsrVarEx[j]%>%<%} else {%>&nbsp;<%}%></span>
                           <span id="spOnly" style="display:none;"></span>
                         </td>
                      <%}%>
                   </td>
