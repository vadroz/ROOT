 ...
 <table border="0" width="100%" height="100%" cellPadding="0" cellSpacing="0">
  ...
  <table class="DataTable" cellPadding="0" cellSpacing="0">
    ...
    <tr>
      ...
      <th class="DataTable2" rowspan="2">Sales<br>%<br>&nbsp;Bonus&nbsp;<br>5%<br>(max)</th>

      <th class="DataTable" colspan="3" id="BAMn01"> </th>
      <th class="DataTable" colspan="1" id="BAMn02"></th>
      <th class="Divdr" rowspan=2>&nbsp;</th>
      <th class="DataTable" colspan="3" id="BAMn11"></th>
      <th class="DataTable" colspan="1" id="BAMn12"></th>
      <th class="Divdr" rowspan=2>&nbsp;</th>
      <th class="DataTable" colspan="3" id="BAMn21"></th>
      <th class="DataTable" colspan="1" id="BAMn22"></th>
      <th class="Divdr" rowspan=2>&nbsp;</th>
      <th class="DataTable" colspan="3" id="BAMn31"></th>
      <th class="DataTable" colspan="1" id="BAMn32"></th>
...
<!------------------------------- Detail Data --------------------------------->
    ...
    <%String sSvReg = " ";
      ...
      <tr class="DataTable">
         ...
         <th class="DataTable">&nbsp;</th>
         <td class="DataTable" id="BAMn0A0<%=i%>" onMouseOver="showAlwTmc(this, '<%=sStrAlwTmcHrs[i][0]%>', '<%=sStrAlwBdgHrs[i][0]%>', '<%=sStrTmcHrs[i][0]%>', '<%=sStrAlwTmcPay[i][0]%>', '<%=sStrAlwBdgPay[i][0]%>', '<%=sStrTmcPay[i][0]%>')" onMouseOut="hideAlwTmc();" nowrap></td>
         <td class="DataTable" id="BAMn0B0<%=i%>" nowrap> </td>
         <td class="DataTable" id="net" nowrap> </td>
         <th class="Divdr">&nbsp;</th>
         <!-- td class="DataTable" id="BAMn0T0<%=i%>" nowrap><%=sTmcHrs[0][i]%></td -->

         <td class="DataTable" id="BAMn1A0<%=i%>" onMouseOver="showAlwTmc(this, '<%=sStrAlwTmcHrs[i][1]%>', '<%=sStrAlwBdgHrs[i][1]%>', '<%=sStrTmcHrs[i][1]%>', '<%=sStrAlwTmcPay[i][1]%>', '<%=sStrAlwBdgPay[i][1]%>', '<%=sStrTmcPay[i][1]%>')" onMouseOut="hideAlwTmc();" nowrap></td>
         <td class="DataTable" id="BAMn1B0<%=i%>" nowrap> </td>
         <td class="DataTable" id="net" nowrap></td>
         <th class="Divdr">&nbsp;</th>
         <!--td class="DataTable" id="BAMn1T0<%=i%>" nowrap><%=sTmcHrs[1][i]%></td -->

         <td class="DataTable" id="BAMn2A0<%=i%>" onMouseOver="showAlwTmc(this, '<%=sStrAlwTmcHrs[i][2]%>', '<%=sStrAlwBdgHrs[i][2]%>', '<%=sStrTmcHrs[i][2]%>', '<%=sStrAlwTmcPay[i][2]%>', '<%=sStrAlwBdgPay[i][2]%>', '<%=sStrTmcPay[i][2]%>')" onMouseOut="hideAlwTmc();" nowrap></td>
         <td class="DataTable" id="BAMn2B0<%=i%>" nowrap></td>
         <td class="DataTable" id="net" nowrap> </td>
         <th class="Divdr">&nbsp;</th>
         <!-- td class="DataTable" id="BAMn2T0<%=i%>" nowrap><%=sTmcHrs[2][i]%></td -->

         <td class="DataTable" id="BAMn3A0<%=i%>" onMouseOver="showAlwTmc(this, '<%=sStrAlwTmcHrs[i][3]%>', '<%=sStrAlwBdgHrs[i][3]%>', '<%=sStrTmcHrs[i][3]%>', '<%=sStrAlwTmcPay[i][3]%>', '<%=sStrAlwBdgPay[i][3]%>', '<%=sStrTmcPay[i][3]%>')" onMouseOut="hideAlwTmc();"nowrap> </td>
         <td class="DataTable" id="BAMn3B0<%=i%>" nowrap> </td>
         <td class="DataTable3"  nowrap></td>
        ...
        <tr class="DataTable1">
          ...
          <%for(int j=0; j < 7; j++ ) {%>
            ...
            <%for(int k=0; k < 4; k++ ) {%>
              ...
              <%}%>
              <%if(j==3) {%>
                <td class="DataTable" onMouseOver="showAlwTmc(this, '<%=sRegAlwTmcHrs[iRow-1][k]%>', '<%=sRegAlwBdgHrs[iRow-1][k]%>', '<%=sRegTmcHrs[iRow-1][k]%>', '<%=sRegAlwTmcPay[iRow-1][k]%>', '<%=sRegAlwBdgPay[iRow-1][k]%>', '<%=sRegTmcPay[iRow-1][k]%>')" onMouseOut="hideAlwTmc();" nowrap id="BAMn<%=k%>AR<%=iRow-1%>"></td>
                <td class="DataTable" nowrap id="BAMn<%=k%>BR<%=iRow-1%>"></td>                
              <%}%>
              ...
              <%if(j < 4 ){%>                
                <td class="DataTable" nowrap  
                  ...
                  <%if(j==0){%>$<%=sRepTot[0][iRow-1][k]%><%} else if(j != 3){%><span id="colPayPrc"><%=sRegAlwVsActPayOver[iRow-1][k]%>%</span><span id="colPayHrs"><%=sRegOverHrs[iRow-1][k]%></span><span id="colPayAmt">$<%=sRegOverPay[iRow-1][k]%></span><%}%></td><!-- Budget Varieance -->
      ...
      <td class="DataTable1" colspan="2">Report Totals</td>
       <%for(int j=0; j < 7; j++ ) {%>
         ...
         <%for(int k=0; k < 4; k++ ) {%>
            ...
            <%}%>
            <%if(j==3) {%>                
                <td class="DataTable" id="BAMn<%=k%>AT" onMouseOver="showAlwTmc(this, '<%=sRegAlwTmcHrs[4][k]%>', '<%=sRegAlwBdgHrs[4][k]%>', '<%=sRegTmcHrs[4][k]%>', '<%=sRegAlwTmcPay[4][k]%>', '<%=sRegAlwBdgPay[4][k]%>', '<%=sRegTmcPay[4][k]%>')" onMouseOut="hideAlwTmc();"></td>
                <td class="DataTable" id="BAMn<%=k%>BT"></td>
              <%}%>

          <%if(j < 4) {%>
            <td class="DataTable" nowrap
                id="<%if(j==2 && k < 3){%>tmc<%} else  if(j==3 && k < 3){%>net<%} else{%>all<%}%>">
                  <%if(j==0){%>$<%=sRepTot[0][4][k]%><%} else if( j != 3) {%><span id="colPayPrc"><%=sRegAlwVsActPayOver[4][k]%>%</span><span id="colPayHrs"><%=sRegOverHrs[4][k]%></span><span id="colPayAmt">$<%=sRegOverPay[4][k]%></span><%}%></td>
             <%if(k < 3 ){%><th class="Divdr">&nbsp;</th><%}%>
          <%}%>
            <%if(j==3) {%>
