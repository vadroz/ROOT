...
{
   response.sendRedirect("SignOn1.jsp?TARGET=EComMarketInclProfile.jsp");
}
else
{
	String sStmt = "Select * from RCI.ECMRKPRF";
	...
	while(runsql.readNextRecord())
	{
	    ...
	    vFflPrc.add(runsql.getData("ffl_prc").trim());
	}       
%>
<HTML>
<HEAD>
<title>EC-Market-Incl-Profile</title>
...
//==============================================================================
function setProfile(prof, qty, amt, prc, action)
{
   	var hdr = "Add Profile";
	...
	if(action =="Dlt")
	{
		...
		document.all.FflPrc.readOnly = true;
	}
...
{	
    ...
    panel += "<tr class='DataTable'>"
       ...
       + "</tr>"
       + "<tr class='DataTable'>"  
       + "<td class='DataTable1' nowrap>Fulfillment Percent: </td>"
       + "<td class='DataTable1' nowrap><input name='FflPrc' maxlength=3 size=3></td>"         
     + "</tr>"
    panel += "<tr class='DataTable'>"
...
function vldProfile(action)
{
	...
	var prc = document.all.FflPrc.value.trim();
	if(prc==""){error=true; msg += br + "The Fulfillment Percent cannot be blank."; br="<br>";}
	else if(isNaN(prc)){error=true; msg += br + "The Fulfillment Percent must be numeric."; br="<br>";}
	else if(eval(prc) < 0){error=true; msg += br + "The Fulfillment Percent cannot be negative."; br="<br>";}
	
	
	if(error){ document.all.tdError.innerHTML = msg; }
	else{ sbmProfile(prof, qty, amt, prc, action); }
}
//==============================================================================
// submit Profile entry
//==============================================================================
function sbmProfile(prof, qty, amt, prc, action)
{
	var url="EComMrktInclPrfSave.jsp?"
      + "prof=" + prof
      + "&qty=" + qty    
      + "&amt=" + amt
      + "&prc=" + prc
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor=moccasin>
    <TD vAlign=top align=middle><B>Retail Concepts Inc.
        <BR>E-Commerce Market Inclusion Profile List
        </B><br>
       ...
       <table border=1 cellPadding="0" cellSpacing="0" id="tbRtvEnt">
         <tr class="DataTable">
             <th class="DataTable">Profile<br>
                <a class="small" href="javascript: setProfile(null,null,null, null,'Add');">New</a>
             </th>
             <th class="DataTable">Minimum Qty</th>
             <th class="DataTable">Minimum Amount</th>
             <th class="DataTable">Fulfillment Percent</th>
         ...
         <tr id="trProd" class="DataTable">
            <td class="DataTable1" nowrap >
               <a class="small" href="javascript: setProfile('<%=vProfile.get(i)%>', '<%=vMinQty.get(i)%>'
                         ,'<%=vMinAmt.get(i)%>','<%=vFflPrc.get(i)%>','Upd');"><%=vProfile.get(i)%></a>
            </td>
            <td class="DataTable2" nowrap ><%=vMinQty.get(i)%></td>
            <td class="DataTable2" nowrap >$<%=vMinAmt.get(i)%></td>
            <td class="DataTable2" nowrap ><%=vFflPrc.get(i)%>%</td>
            <td class="DataTable2" nowrap >
               <a class="small" href="javascript: setProfile('<%=vProfile.get(i)%>', '<%=vMinQty.get(i)%>'
                         ,'<%=vMinAmt.get(i)%>','<%=vFflPrc.get(i)%>','Dlt');">Delete</a>
            </td>
         </tr>
       <%}%>
