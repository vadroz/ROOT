...
else
{
   ...
   if(!sPonum.trim().equals("") || !sAlloc.trim().equals("") || !sReceipt.trim().equals(""))
   {
  	...
  	//out.print("RecSrc: " + sRcdSrc + "<br>IssStr: " + sIStore + "<br>DstStr: " + sDStore
...
function setAcknowl(ist, dst, ctn, sku, qty, cell)
{
   ...
   document.all.dvPrompt.style.pixelTop= document.documentElement.scrollTop + 60;
...
function popItemPanel(ist, dst, ctn, sku)
{
  ...
  // }
   ...
   if(sku!="ALL")
   {
      panel += "<tr><td class='Prompt' nowrap>Sku:</td>"
             + "<td class='Prompt' nowrap><input class='Small' name='Sku' size=10>"
             + "</td>"
          + "</tr>"
...
function  Validate(type)
{
   ...
   if(type != "ALL")
   {
     ...
     if(type == "NEW")
     {
        if (sku == "") { error = true; msg += "SKU is not entered.\n" }
        else if (isNaN(sku))  { error = true; msg += "SKU is not numeric.\n" }
        else if (eval(sku) < 0)  { error = true; msg += "SKU should not be negative.\n" }
     }
...
function confAcknowl(msg, ist, dst, ctn, exp, sku, qty, action)
{
   ...
   document.all.dvPrompt.innerHTML = html;
   document.all.dvPrompt.style.width = 400;
   document.all.dvPrompt.style.pixelLeft= document.documentElement.scrollLeft + 250;
   document.all.dvPrompt.style.pixelTop= document.documentElement.scrollTop + 60;
   document.all.dvPrompt.style.visibility = "visible";
...
<p align=center>
      ...
      <table class="DataTable" cellPadding="0" cellSpacing="0">
        <tr>
	    <th class="DataTable" rowspan=2><a href="DstStsInq.jsp?RecSrc=<%=sRcdSrc%>&IStore=<%=sIStore%>&IStrName=<%=sIStrName%>&DStore=<%=sDStore%>&DStrName=<%=sDStrName%>&Distro=<%=sDistro%>&FromDate=<%=sFrom%>&ToDate=<%=sTo%>&Status=<%=sStatus%>&Sort=ISTR">Issuing<br>Store</a></th>
            <th class="DataTable" rowspan=2><a href="DstStsInq.jsp?RecSrc=<%=sRcdSrc%>&IStore=<%=sIStore%>&IStrName=<%=sIStrName%>&DStore=<%=sDStore%>&DStrName=<%=sDStrName%>&Distro=<%=sDistro%>&FromDate=<%=sFrom%>&ToDate=<%=sTo%>&Status=<%=sStatus%>&Sort=DSTR">Dest<br>Store</a></th>
            <th class="DataTable" rowspan=2>Document<br>Number</th>
            <th class="DataTable" rowspan=2>Dist<br>Date</th>
            <th class="DataTable" rowspan=2>Carton</th>
            <th class="DataTable" rowspan=2>S<br>r<br>c</th>
            <th class="DataTable" rowspan=2>Status</th>
            <th class="DataTable" rowspan=2>Status<br>Date</th>
            <th class="DataTable" rowspan=2><a href="DstStsInq.jsp?RecSrc=<%=sRcdSrc%>&IStore=<%=sIStore%>&IStrName=<%=sIStrName%>&DStore=<%=sDStore%>&DStrName=<%=sDStrName%>&Distro=<%=sDistro%>&FromDate=<%=sFrom%>&ToDate=<%=sTo%>&Status=<%=sStatus%>&Sort=DIV">Div<br>#</a></th>
            <th class="DataTable" rowspan=2>Dpt<br>#</th>
            <th class="DataTable" rowspan=2>Cls<br>#</th>
            ...
            <th class="DataTable" rowspan=2><a href="DstStsInq.jsp?RecSrc=<%=sRcdSrc%>&IStore=<%=sIStore%>&IStrName=<%=sIStrName%>&DStore=<%=sDStore%>&DStrName=<%=sDStrName%>&Distro=<%=sDistro%>&FromDate=<%=sFrom%>&ToDate=<%=sTo%>&Status=<%=sStatus%>&Sort=VEN">Vendor</a></th>
            <th class="DataTable" rowspan=2>Orig<br>Qty</th>
            <th class="DataTable" rowspan=2>Retail</th>
            <th class="DataTable" rowspan=2>P.O.<br>#</th>
            <th class="DataTable" rowspan=2>Receipt<br>#</th>
            <th class="DataTable" rowspan=2>Pick<br>#</th>
            <th class="DataTable" rowspan=2>Alc<br>#</th>
            <th class="DataTable" rowspan=2>Shipment<br>#</th>
            <th class="DataTable" rowspan=2>Iss Str<br>Onhand</th>
            <th class="DataTable" rowspan=2>Dst Str<br>Onhand</th>
            
            <%if(bAllow){%>
               <th class="DataTable" rowspan=2>A<br>c<br>k<br>n<br>l</th>
            <%}%>
               <!--th class="DataTable">Acknowl.<br>Type</th -->
               <th class="DataTable" rowspan=2>Acknowl.<br>Qty</th>
      ...
      %>
             ...
             <tr class="<%if(!sSku.trim().equals("TOTAL")){%>Row<%} else {%>Row1<%}%>"
               ...
               <%if(!sSku.trim().equals("TOTAL")){%>
                 ...
                 <td class="Cell" nowrap><%=sIssOnh%></td>
                 <td class="Cell" nowrap><%=sDstOnh%></td>
               <%}
               ...
               <%if(bAllow){%>
                  <td id="tdAckn<%=i%>" class="Cell" nowrap>
                    <a onclick="needToConfirm=false;chgObjAcknol('<%=i%>')" href="javascript: setAcknowl('<%=sIssStr%>', '<%=sDstStr%>', '<%=sDocNum%>', '<%if(sSku.trim().equals("TOTAL")){%>ALL<%} else {%><%=sSku%><%}%>', '<%=sQty%>', 'tdAckn<%=i%>' )">A</a>
                  </td>
               <%}%>
                  <!-- td class="Cell" nowrap><%if(sSku.trim().equals("TOTAL")){%>&nbsp;<%=sExcept%><%} else {%>&nbsp;<%}%></td -->
                  <!-- td class="Cell1" nowrap><%if(!sExcept.equals("")){%><%=sAcknQty%><%} else {%>&nbsp;<%}%></td -->
                  <td class="Cell1" nowrap><%if(!sAcknQty.trim().equals("")){%><%=sAcknQty%><%} else {%>&nbsp;<%}%></td>
                  <td class="Cell1" nowrap><%if(!sDscQty.trim().equals("0")){%><%=sDscQty%><%} else {%>&nbsp;<%}%></td>
