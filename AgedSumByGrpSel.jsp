...
<%
   ...
   {
     response.sendRedirect("SignOn1.jsp?TARGET=AgedSumByGrpSel.jsp&APPL=ALL");
   }
   else
   {
...
//store
  ...
  if (error) alert(msg);
  else{ sbmSlsRep(divarr, dpt, cls, selstr, level, ucr, prec, nummon); }
  return error == false;
}
//==============================================================================
// Submit Stock Position Report
//==============================================================================
function sbmSlsRep(div, dpt, cls, str, level, ucr, prec, nummon)
{
  var url = null;
  url = "AgedSumByGrp.jsp?"
  url += "&Dpt=" + dpt
      + "&Cls=" + cls
      + "&Ven=ALL"
      + "&Level=" + level
