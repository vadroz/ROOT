...
<%
	...
	String sStmt = "Select "
	  + " AWSITE,AWORD,AWDTLID,digits(AWCLS) as AWCLS, digits(AWVEN) as AWVEN, digits(AWSTY) as AWSTY" 
	  ...
	  + ",ides"	  
      + ",case when (select '1' from rci.EcAlcWks where AsSite=AwSite and AsOrd=AwOrd"
      + " fetch first 1 row only) is null then 'N' else 'Y' end as comb"       
	  + " from RCI.ECALCWK"
	  + " left join rci.EcOrPasV1 on v1site=AwSite and v1ord=AwOrd and v1sku=AwSku and v1str=AwStr"
...
<HTML>
<HEAD>
<title>Ec Test Pick Logic</title>
...
{
   var url = "EComTestPickLogicShip.jsp?ord=" + ord;
