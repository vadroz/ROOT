...
function Validate(lvl)
{
  ...
  var clearance = "N";
  if(document.all.InclClrSls.checked){ clearance = "Y"; }  

  //var lvl = getLevel(str, div, dpt, cls)
  
  if (error) alert(msg);
  else { sbmReport(div, dpt, cls, ven, vennm, str, frdate, todate, lvl, type, clearance); }
...
// submit report
//==============================================================================
function sbmReport(div, dpt, cls, ven, vennm, str, frdate, todate, lvl, type, clearance)
{	  	
	url = "WkSlsTrendVen.jsp?"
    	+ "&Dpt=" + dpt
    	+ "&Cls=" + cls
    	+ "&Ven=" + ven
...
function setAll(on)
{
   ...
   for(var i=0; i < str.length; i++) 
   { 
	   var sstr = str[i].value;
	   
	   if(sstr != "1" && sstr != "2" && sstr != "12" && sstr != "13" && sstr != "56"
		   && sstr != "59" && sstr != "72" && sstr != "76" && sstr != "99"
			   && sstr != "100" && sstr != "101" && sstr != "102" && sstr != "199")
	   {
	   		str[i].checked = on;
	   }
   }
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor=moccasin>
    <TD vAlign=top align=middle><B>Retail Concepts Inc.
        <BR>Sales Trend Comparison <sup style="color:red;">(new)</sup> - Selection</B>
      ...
      <TABLE>
        ...
        <TR>
          <TD align=center colspan=4>
              ...
              <br>
              <input name="InclClrSls" type="checkbox" value="Y">Clearance Report (shows only clearance data)             
          </TD>
        </TR>
        <TR>
            <TD align=center colSpan=4>
               <button onClick="Validate('S')">Display By Store</button>&nbsp;&nbsp;
               <button onClick="Validate('D')">Display By Merchandise</button>&nbsp;&nbsp;
               <button onClick="Validate('V')">Display By Vendor</button>
           </TD>
          </TR>
         </TBODY>
        </TABLE>
