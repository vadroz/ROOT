<%@ page import="java.util.*, onhand01.ItemBsrEditV2"%>
<%
   ...
   ItemBsrEditV2 itmbsr = new ItemBsrEditV2(sSrchDiv,  sSrchDpt, sSrchCls, sSrchVen);
   int iNumOfStr = itmbsr.getNumOfStr();
   int iNumOfCvs = itmbsr.getNumOfCvs();
...
<html>
<head>
<title>Item BSR Editing(new)</title>
<style>
     ...
     <tr bgColor="moccasin">
      <td ALIGN="center" VALIGN="TOP">
       ...
       <br>
        ...
        <a href="ItemBSREditSelV2.jsp"><font color="red" size="-1">Selection</font></a>;&#62
        ...
        <%
        System.out.println("iNumOfCvs=" + iNumOfCvs);
        ...
        for(int i=0; i < iNumOfCvs; i++)
        {
            ...
            String sChnQty = itmbsr.getChnQty();
            String sRet = itmbsr.getRet();
            String sOrigRet = itmbsr.getOrigRet();
            String sSlsUnit = itmbsr.getSlsUnit();
            String sCost = itmbsr.getCost();
            String sFstRctDt = itmbsr.getFstRctDt();
            String sLstRctDt = itmbsr.getLstRctDt();
            String sMkdnDt = itmbsr.getMkdnDt();
            ...
            String [] sSts = itmbsr.getSts();
            String [] sSellOff = itmbsr.getSellOff();
            String sTotSellOff = itmbsr.getTotSellOff();
            String sChnOnh = itmbsr.getChnOnh();
            String sLstSlsDt = itmbsr.getLstSlsDt();        
            
            itmbsr.setNumOfItm();        		
        	int iNumOfItm = itmbsr.getNumOfItm();
        	System.out.println("iNumOfItm=" + iNumOfItm);
        %>
        	...
        	for(int j=0; j < iNumOfItm; j++)
        	{
        		...
        		sStock = itmbsr.getStock();
        		sIdeal = itmbsr.getIdeal();
            	sMax = itmbsr.getMax();
            	sMin = itmbsr.getMin();
        	...
        	%>
              ...
              <!-- ------------ Division line ------------------------- -->
              <%if(bBreak){%><tr class="Break"><td class="Break" colspan=7>&nbsp</td><%for(int k=0; k < iNumOfStr; k++){%><td class="Break" colspan=3><%=sStr[k]%></th>
               <%}%><%}%>
              ...
              <tr id="trIdeal<%=iNumOfItm%>" class="DataTable1<%if(sSrchItem.equals(sItem)){%>1<%}%>">
                ...
                <%for(int k=0; k < iNumOfStr; k++){%>
                    <td id="tdMin<%=iNumOfItm%>" class="DataTable1a"><%if(!sMin[k].equals("")){%><%=sMin[k]%><%} else {%>&nbsp;<%}%></td>
                    <td id="tdIdeal<%=iNumOfItm%>" class="DataTable1b"><%if(!sIdeal[k].equals("")){%><%=sIdeal[k]%><%} else {%>&nbsp;<%}%></td>
                    <td id="tdMax<%=iNumOfItm%>" class="DataTable11a"><%if(!sMax[k].equals("")){%><%=sMax[k]%><%} else {%>&nbsp;<%}%></td>
                <%}%>`
              ...
              <tr class="DataTable1">
                ...
                <%for(int k=0; k < iNumOfStr; k++){%>
                    <td class="DataTable1" colspan=2><%if(!sStock[k].equals("")){%><%=sStock[k]%><%} else {%>&nbsp;<%}%></td>
                    <td class="DataTable11"><%if(!sSales[k].equals("")){%><%=sSales[k]%><%} else {%>&nbsp;<%}%></td>
                <%}%>
              </tr>
            <%iNumOfItm++;%>
          	<%}%>
