...
{	
	...
	ctlinfo.setSkuLst(sSelStr, sSelSts, sWkend, sWkend2, sYear, sMonth, sDateLvl
			,  sReas, sType, sDefect, sSelVen, sSort, sUser);
...
$(window).bind("scroll", function() {
	...
	{
		var desc = "#tdDesc" + i;
		$(desc).css({ left: posLeft, position : "relative", background:color, display: "block" });
		$(desc).css("z-index", 0);
...
<!-------------------------------------------------------------------->
      ...
      <table class="tbl02">
        <tr id="trHdr01" class="trHdr01">
        	...
        	<th id="thDesc" class="th02" rowspan=2><a href="javascript: sortby('Desc')">Description</th>
        	...
        	<th class="th02" rowspan=2>Chain Onhand</th>
...
<!------------------------------- order/sku --------------------------------->
           ...
           <tr id="trGrp<%=i%>" class="<%=sTrCls%>">
             ...
             <td id="tdDesc<%=i%>" class="td67" nowrap><%=sDesc%></td>
             ...
             <td id="tdTotal<%=i%>" class="td12" nowrap>
