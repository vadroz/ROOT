<%@ page import="java.util.*, java.text.*, payrollreports.BfdgSchActMonAllStr"%>
<%
...
{
   response.sendRedirect("SignOn1.jsp?TARGET=BfdgSchActMonAllStr.jsp&APPL=" + sAppl + "&" + request.getQueryString());
}
else
{
    BfdgSchActMonAllStr bdgwkall = new BfdgSchActMonAllStr(sYear, sMonth, sUser);
...
<!-------------------------------------------------------------------->
   <table border="0" width="100%" cellPadding="0" cellSpacing="0">
     <tr bgColor="ivory">
      ...
      <br>Monthly Budget vs. Schedule and Actual Payroll (New Schedule)
      <br>Month: <%=sMonName%>/<%=sYear%> (thru w/e <%=sPassedWkend%>)
      </b>
      ...
      <table border=1 cellPadding="0" cellSpacing="0" >
        ...
        <tr>
          ...
          <th class="DataTable2">Hours Earned (Based on Salaried Employees on V or H) </th>
     ...
     <%for(int i=0; i < iNumOfReg; i++){%>
        ...
        %>
            ...
            <td class="DataTable"><a href="BfdgSchActMn.jsp?Store=<%=sStr[iStrNum]%>&Year=<%=sYear%>&Month=<%=sMonth%>&MonName=<%=sMonName%>" target="_blank"><%=sStr[iStrNum]%></a></td>
            <th class="DataTable">&nbsp;</th>
