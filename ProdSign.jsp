<%@ page import="salesigns01.ProdSign, java.util.*, java.text.*, java.io.*"%>
<%
   ...
   if (sSelSty == null){ sSelSty = " "; }
   //----------------------------------
   // Application Authorization
   //----------------------------------
   if (session.getAttribute("USER")==null)
   {
     response.sendRedirect("SignOn1.jsp?TARGET=ProdSign.jsp&APPL=ALL&" + request.getQueryString());
   }
   else
   {

   String sUser = session.getAttribute("USER").toString();
   ProdSign prods = new ProdSign(sSelCls, sSelVen, sSelSty, sUser);
%>
<html>
<head>
<title>Product Signs</title>
...
<!-------------------------------------------------------------------->
    ...
    <tr>
      ...
      <a href="ProdSignSel.jsp"><font color="red" size="-1">Selection</font></a>&#62;This Page.
      <br>
  <!----------------------- Order List ------------------------------>
  ...
  <!-------------------------- Order List ------------------------------->
      <%
       while(prods.getNext())
       {
          ...
          String sLink = "/Signs/Products/Division " + sDiv + "/"
              + sCls + "." + sVen + "." + sSty + "." + sClr;
      ...
      %>
         <tr class="DataTable1">
           <td class="DataTable"><a href="<%=sLink%>" target="_blank"><%=sCls%>-<%=sVen%>-<%=sSty%>
