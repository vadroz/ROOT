<%@ page import="rciutility.StoreSelect, payrollreports.PrMonStrBdgSch, java.util.*, java.text.*"%>
<%
  ...
  {
     response.sendRedirect("SignOn1.jsp?TARGET=PrMonStrBdgSch.jsp&APPL=" + sAppl + "&" + request.getQueryString());
  }
   else {
  ...
  PrMonStrBdgSch monsch  = new PrMonStrBdgSch("STR", "1", "N", sExclude70, sUser);
...
//==============================================================================
{
   var SbmString = "PrMonStrBdgSch.jsp";
...
function inclExcl70()
{
  var url = "PrMonStrBdgSch.jsp?STORE=<%=sSelStore%>"
      ...
      <table class="DataTable" align="center" >
           ...
           %>
            <tr>
                 ...
                 <td class="DataTable" nowrap>
                    <a href="PrWkSched.jsp?STORE=<%=sSelStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeek[i]%>" target="_blank">
                              <%=sWeek[i]%></a></td>
                 ...
                 <th class="DataTable">
                    <%if(!sSelStore.equals("ALL")){%><a href="BdgSchActWk.jsp?Store=<%=sSelStore%>&StrName=<%=sThisStrName%>&Wkend=<%=sWeek[i]%>" target="_blank">AB</a><%}
                     else {%><a href="BdgSchActWkAllStr.jsp?Wkend=<%=sWeek[i]%>" target="_blank">AB</a><%}%>
                 </th>
                 ...
                 <%if(sWkActual[i].equals("1")){%>
                     <%if(sSelStore.equals("ALL")){%><th class="DataTable"><a href="PrActAvgVarAllStr.jsp?From=BEGWEEK&To=<%=sWeek[i]%>" target="_blank">V</a></th><%}
                     else { %><th class="DataTable"><a href="PrActAvgVar.jsp?Store=<%=sSelStore%>&StrNm=<%=sThisStrName%>&From=BEGWEEK&To=<%=sWeek[i]%>" target="_blank">V</a></th><%}%>
                 <%}
         ...
         <!-- ************************************************************ -->
            ...
            <tr>
               ...
               <th class="DataTable">
                    <%if(!sSelStore.equals("ALL")){%><a href="BdgSchActMn.jsp?Store=<%=sSelStore%>&StrName=<%=sThisStrName%>&Year=<%=sYear%>&Month=<%=sMonNum%>&MonName=<%=sMonName[iMonNum]%>" target="_blank">AB</a><%}
                     else {%><a href="BdgSchActMonAllStr.jsp?Store=<%=sSelStore%>&StrName=<%=sThisStrName%>&Year=<%=sYear%>&Month=<%=sMonNum%>&MonName=<%=sMonName[iMonNum]%>" target="_blank">AB</a><%}%>
               </th>
