<%@ page import="rciutility.StoreSelect, rciutility.RunSQLStmt, java.sql.*, java.util.*"%>
<%
   ...
   {
     response.sendRedirect("SignOn1.jsp?TARGET=EComSrlAsgSel.jsp&APPL=ALL");
   }
   else
   {
    ...
    sStmt = "Select * from RCI.ECSHPMTH order by SMSHPMTH";            
	runsql = new RunSQLStmt();
	runsql.setPrepStmt(sStmt);
	rs = runsql.runQuery();
	Vector vShpMeth = new Vector();
	Vector vShpMethDesc = new Vector();
	while(rs.next())
	{
		vShpMeth.add(rs.getString(1));
		vShpMethDesc.add(rs.getString(2));
	}
%>
...
function Validate()
{
  ...
  var meth = document.all.ShpMeth;
  selmth = new Array();
  var nummth = 0
  for(var i=0; i < meth.length; i++)
  {
    if(meth[i].checked){ selmth[nummth] = meth[i].value; nummth++; }
  }
  if (nummth == 0){ error=true; msg+="\nAt least 1 shipping method must be selected.";}
  ...
  if (error) alert(msg);
  else{ sbmPlan(stsfrdate, ststodate, selstr, sku, ord, selmth) }
  return error == false;
}
//==============================================================================
// Submit OTB Planning
//==============================================================================
function sbmPlan(stsfrdate, ststodate, selstr, sku, ord, selmth)
{
  var url = null;
  url = "EcomFxUnship.jsp?"
      + "FrDate=" + stsfrdate
      + "&ToDate=" + ststodate
      + "&Sku=" + sku
      + "&Ord=" + ord
  ...
  for(var i=0; i < selmth.length; i++){ url += "&Mth=" + selmth[i]; }
...
//check All Status
//==============================================================================
function setMeth(check)
{
	var meth = document.all.ShpMeth;
	for(var i=0; i < meth.length; i++) { meth[i].checked = check; }
}
</script>
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor=moccasin>
    <TD vAlign=top align=middle><B>Retail Concepts Inc.
        <BR>E-Commerce - FedEx is not Shipped but packed - Selection</B>
        <br><a href="/"><font color="red" size="-1">Home</font></a>&#62;This Page
      ...
      <TABLE>
        ...
        <TR><TD style="border-bottom:darkred solid 1px" colspan="5" >&nbsp;</TD></TR>
        <TR><TD class="Cell2" colspan=5>Select Shipping Methods</TD></tr>
        
        <TR><TD colspan=5>
        	<table>
        		<%for(int j=0; j < vShpMeth.size();j++){%>
            		<%if(j==0 || j%5==0){%><tr><%}%>
            		<td style="font-size:12px">
            		   <input class="Small" type="checkbox" name="ShpMeth" value="<%=vShpMeth.get(j)%>">
            		       <%=vShpMeth.get(j)%> - <%=vShpMethDesc.get(j)%>
            		</td>
        		<%}%>
        	</table>
        	<br><button class="Small" onClick="setMeth(true)">All</button> &nbsp;
        	    <button class="Small" onClick="setMeth(false)">Reset</button> &nbsp;
        </TD></tr>
