<%@ page import="payrollreports.PsEmpNum"%>
<% String sStore = request.getParameter("STORE");
 ...
 // get employee number by hours
   PsEmpNum EmpNum = new PsEmpNum(sStore, sWeekDay, "WEEK");
...
<style>body {background:ivory;}
        ...
        th.DataTable1 { background:#FFCC99; border-top: darkred solid 1px;border-left: darkred solid 1px;
...
<!-------------------------------------------------------------------->
      ...
      <table class="DataTable" cellPadding="0" cellSpacing="0">
         <tr>
           ...
           <%for(int i=0; i < 7; i++ ){%>
             <th class="DataTable" colspan="<%=iNumOfSec+1%>"><%=sWeek[i]%>
                <br><%=sDaysOfWeek[i]%></th>
         ...
         <tr><th class="DataTable" rowspan="1">Time</th>
           <%for(int i=0; i < 7; i++ ){%>
                <%for(int j=0; j < iNumOfSec; j++ ){%>
                    <th class="DataTable1"><%=sSecName[j]%></th>
                <%}%>
                <th class="DataTable1">Total</th>
