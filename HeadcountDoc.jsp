...
<%
   String sPath = "C:/Program Files/Apache Software Foundation/Tomcat 7.0/webapps/ROOT/HeadcountRep/";
//----------------------------------
// Application Authorization
//----------------------------------
if (session.getAttribute("USER")==null)
{
   response.sendRedirect("SignOn1.jsp?TARGET=HeadcountDoc.jsp&APPL=ALL");
}
else
{
   File dir = new File(sPath);
   if (!dir.exists()) {  dir.mkdirs(); }
   File[] fdocum = dir.listFiles();
%>
   ...
   var html = ""
     + "<table width='100%' cellPadding='0' cellSpacing='0'>"
       ...
       + "</td></tr>"
           + "<tr colspan='2'>"
           + "<td class='Dtl3'>"
           + "<form name='Upload'  method='post'  enctype='multipart/form-data' action='HcDocUpload.jsp'>"
...
<p align="center"> <b>RETAIL CONCEPTS, INC.
                   <br>Headcount Statistics Reports</b><br>
  ...
  <table class="DataTable" align="center" >
     ...
     <!-- File List -->
     <!-- ================================================================== -->
       <%for(int i=0; i<fdocum.length;i++){%>
          <tr>
            <!-- show link if directory -->
            <%if(fdocum[i].isDirectory()) {%>
               <td class="DataTable" colspan=2>
                 <a href="ServerSignList.jsp?Path=<%=sPath + "/" + fdocum[i].getName()%>"><%=fdocum[i].getName()%></a>
               </td>
            <%}
              else {%>
                 <td class="DataTable" >
                     <a href="HeadcountRep/<%=fdocum[i].getName()%>"><%=fdocum[i].getName()%></a>
                 </td>
                 <td class="DataTable" >
                  <a href="javascript: dltFile('<%=sPath + "/" + fdocum[i].getName()%>')">D</a>
               </td>
            <%}%>
       <%}%>
      </td>
