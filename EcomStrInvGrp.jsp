<%@ page import="ecommerce.EcomStrInvGrp, java.util.*"%>
<%
...
{
    response.sendRedirect("SignOn1.jsp?TARGET=EcomStrInvGrp.jsp");
}
else
{
    String sStrAllowed = session.getAttribute("STORE").toString();
    String sUser = session.getAttribute("USER").toString();
    EcomStrInvGrp strinvl = new EcomStrInvGrp(sSelStr, sSelDiv, sSelDpt, sSelCls, sSelVen, sUser);
...
//submit single Item
//==============================================================================
function chgStrGrp(div, dpt, cls, ven, str, exists, profile, qty, amt, action)
{
...
function vldStrGrp(div, dpt, cls, ven, str, action)
{
	...
	else{ sbmStrGrp(div, dpt, cls, ven, str, prof, qty, amt, action); }
}
//==============================================================================
// submit single Item
//==============================================================================
function sbmStrGrp(div, dpt, cls, ven, str, prof, qty, amt, action)
{
   var url = "EcomStrInvGrpSv.jsp?"
       + "Div=" + div
       + "&Dpt=" + dpt
       + "&Cls=" + cls
       + "&Ven=" + ven
...
<!-------------------------------------------------------------------->
      ...
      <br>Included Store Inventory (Pick Logic)
      <br></b>
      <a href="/"><font color="red" size="-1">Home</font></a>&#62;
      <a href="EcomStrInvGrp.jsp"><font color="red" size="-1">All Divisions</font></a>&#62;
      ...
      <table border=1 class="DataTable" id="tbSku" cellPadding="0" cellSpacing="0">
        ...
        <tr>
          <%for(int i=0; i < iNumOfStr; i++){%>
             <th class="DataTable">
                <a href="EcomStrInvGrp.jsp?Div=<%=sSelDiv%>&Dpt=<%=sSelDpt%>&Cls=<%=sSelCls%>&Str=<%=sStrLst[i]%>"><%=sStrLst[i]%></a>
             </th>
          <%}%>
        </tr>
        <tr>
          <%for(int i=0; i < iNumOfStr; i++){%>
               <th class="DataTable"><a href="javascript: chgStrGrp('<%=sSelDiv%>', '<%=sSelDpt%>', '<%=sSelCls%>', '<%=sSelVen%>', '<%=sStrLst[i]%>',null, null,null,null,'STR_UPD')">Chg</a></th>           
          <%}%>
        </tr>
<!------------------------------- Data Detail --------------------------------->
           ...
           %>
              <tr class="DataTable" id="trSku<%=i%>" onclick="hiliLine(this)">
                <td class="DataTable2" nowrap>&nbsp;<%if(!sDiv.equals("0")){%><a href="EcomStrInvGrp.jsp?Div=<%=sDiv%>&Str=<%=sSelStr%>"><%=sDiv%></a><%}%>
                    <span class="spnSup"><%=sDrill%></span>
                </td>
                <td class="DataTable2">&nbsp;<%if(!sDpt.equals("0")){%><a href="EcomStrInvGrp.jsp?Div=<%=sDiv%>&Dpt=<%=sDpt%>&Str=<%=sSelStr%>"><%=sDpt%></a><%}%></td>
                <td class="DataTable2">&nbsp;<%if(!sCls.equals("0")){%><a href="EcomStrInvGrp.jsp?Div=<%=sDiv%>&Dpt=<%=sDpt%>&Cls=<%=sCls%>&Str=<%=sSelStr%>"><%=sCls%></a><%}%></td>
                <td class="DataTable2">&nbsp;<%if(!sVen.equals("0")){%><%=sVen%><%}%></td>
                
                <%for(int j=0; j < iNumOfStr; j++){%>
                   <td class="DataTable" id="tdSku<%=i%>" nowrap>                      
                     <a href="javascript: chgStrGrp('<%=sDiv%>', '<%=sDpt%>', '<%=sCls%>', '<%=sVen%>', '<%=sStrLst[j]%>','<%=sStr[j]%>', '<%=sProfile[j]%>','<%=sQty[j]%>','<%=sAmt[j]%>','Add_Upd')">
                         <%if(sStr[j].equals("1")){%>
