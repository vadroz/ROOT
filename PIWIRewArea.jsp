...
<%
  ...
  PiWiRew invrep = new PiWiRew(sStore, "ALL", "A", sPiYearMo.substring(0, 4), sPiYearMo.substring(4), "Y", "ALL");
  invrep.setAllArea();
...
<!------------- end of store selector ---------------------->
        ...
        <a href="PIWIRewSel.jsp"><font color="red">Selection</font></a>&#62;
        This page
      ...
      <table class="DataTable" width="50%"  align="center" cellPadding="0" cellSpacing="0">
        ...
        <%for(int i=0; i < iNumOfArea; i++){%>
           <tr <%if(!sQty[i].equals("Missed Area")){%>class="Count"<%}%>>
             <td class="DataTable">
                <a href="PIWIRew.jsp?STORE=<%=sStore%>&AREA=<%=sArea[i]%>&PICal=<%=sPiYearMo%>&Mbr=<%=sMbr[i]%>"><%=sArea[i]%></a></td>
