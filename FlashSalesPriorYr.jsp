<%@ page import="flashreps.SalesTyPriorYr, flashreps.RcvSalesDate, java.util.*, java.text.SimpleDateFormat"%>
<%
   ...
   if (bAvail)
   {
      ...
      SalesTyPriorYr slsTyLy = new SalesTyPriorYr(sDivision, sDepartment, sClass, sStore, sDate,
      ...
      String sColName = "Str";
      if (sLevel.substring(0, 1).equals("D")) { sColName = "Div"; }
      if (sLevel.substring(0, 1).equals("P")) { sColName = "Dpt"; }
      if (sLevel.substring(0, 1).equals("C")) { sColName = "Cls"; }

      String sLyCol = "LY";
      if (sPeriod.substring(5).equals("PLAN")){ sLyCol = "Plan"; }
      else if(sLevel.substring(1).equals("LY2")){ sLyCol = "LLY"; }
      else if(!sLevel.substring(1).equals("LY")){ sLyCol = "Avg"; }


      String sDispBy = null;

      if (!sLevel.substring(0, 1).equals("S") && !sLevel.equals("T")) sDispBy = "Str";
      ...
      boolean bKioskAllows = (sPeriod.equals("MTDTYLY") || sPeriod.equals("YTDTYLY"))
        && sLevel.substring(0, 1).equals("S") && sDivision.equals("ALL") && sDepartment.equals("ALL") && sClass.equals("ALL");
      ...
      if (sLevel.length() > 1)
      {
        if(sLevel.substring(1).equals("LY2")){ sCompName = "LLY"; }
        else if (sLevel.substring(1).equals("LY2A")){ sCompName = "2 LY Average"; }
        else if (sLevel.substring(1).equals("LY3A")){ sCompName = "3 LY Average"; }
        else if (sLevel.substring(1).equals("LY4A")){ sCompName = "4 LY Average"; }
        else if (sLevel.substring(1).equals("LY5A")){ sCompName = "5 LY Average"; }
      }
%>
<html>
<head>
...
function reverseLevel()
{
   url = "FlashSalesPriorYr.jsp?"
       ...
       var sublvl = "";
       if (Level.length > 1){ sublvl = Level.substring(1) }

       if (Level.substring(0,1) == "S") url += "&Level=D" + sublvl
       else if (Level=="T") url += "&Level=V"
       else if (Level=="V") url += "&Level=T"
       else url += "&Level=S" + sublvl
...
function popPanel()
{
   ...
   var panel = "<table cellPadding='0' cellSpacing='0'  width='100%'>"
      ...
      + "<tr><td class='Prompt'' nowrap colspan=2>"
         + "Prior Years Selection:<br>"
         + "<input name='LYSeL' type='radio' value='' >TY vs. LY &nbsp;&nbsp;"
         + "<input name='LYSeL' type='radio' value='LY2'>TY vs. LLY &nbsp;&nbsp;"
...
<!-------------------------------------------------------------------->
     ...
     <tr bgColor="ivory">
      ...
      <a href="<%if(!bKiosk){%>index.jsp<%} else {%>Outsider.jsp<%}%>"><font color="red" size="-1">Home</font></a>&#62;
          <a href="FlashSalesPriorYrSel.jsp"><font color="red" size="-1">Select Report</font></a>&#62;
...
<!------------------------------- Data Detail --------------------------------->
      <%for(int i=0, str=0, reg=0; i < iNumOfGrp; i++) {%>
        ...
        <%if(sLevel.substring(0, 1).equals("S") && sSort.equals("GRP")) {%>
    ...
    <%if(sStore.equals("ALL") && sLevel.substring(0, 1).equals("S")){%>
      <%for(int i=0; i < 2; i++) {%>
        <tr class="DataTable5">
           <%if(sStore.equals("ALL") && sLevel.substring(0, 1).equals("S")){%>
    ...
    <%if(sDivision.equals("ALL") && !sLevel.equals("T") && !sLevel.equals("V")){%>
      <%for(int i=0; i < iNumOfDmm; i++) {%>
        <tr class="DataTable6" style="background:<%=sDmmColor[i]%>;">
         ...
         <td class="DataTable6" <%if(sLevel.substring(0, 1).equals("D")) {%> onMouseOver="getTool(this, '<%=i%>')"<%}%>
    ...
    <!------------------- Regional Total ------------------------------------>
    <%if(sLevel.substring(0, 1).equals("S") && !sStore.substring(0, 3).equals("RCS")) {%>
    ...
    <%if(sLevel.substring(0, 1).equals("S")) {%>
      <%for(int i=0; i < iNumOfReg; i++) {%>
        <tr class="DataTable1">
    ...
    <!------------------- Comp Store Totals -------------------------------------->
    <%if(!sStore.equals("COMP") && !sStore.equals("COMPE") && sLevel.substring(0, 1).equals("S"))
    {
        for(int i=0; i < 2; i++)
        {
...
<script>
   ...
   window.location.href = "FlashSalesPriorYrSel.jsp";
</script>
<%}%>