...
<%
  ...
  String sAppl = "PAYROLL";
   ...
   {
     response.sendRedirect("SignOn1.jsp?TARGET=SchedbyWeek.jsp&APPL=" + sAppl + "&" + request.getQueryString());
   }
   else
   {
...
@media screen
{
        td.DataTable { background:#e7e7e7; cursor: hand; padding-top:3px; padding-bottom:3px; border-bottom: #FFE4C4 solid 1px; border-right: darkred solid 1px; text-align:left; font-family:Arial; font-size:10px }
	td.DataTable1 { background:#e7e7e7; cursor: hand; padding-top:3px; padding-bottom:3px; border-bottom: #FFE4C4 solid 1px; border-right: darkred solid 1px; text-align:center; font-family:Arial; font-size:10px }
...
if (event.srcElement.className=="Grid"
   ...
   while (obj.offsetParent){
     if (obj.id=="menu" || obj.id=="tooltip2" || obj.id=="dvClinics")
     {
       z=obj;
       break;
     }
...
function showClinicWeekly(name, date, brand)
{
  ...
  document.all.dvClinics.style.pixelTop=110;
      ...
      <%for(int i=0; i < iNumOfMgr; i++){%>
         <tr >
          <td class="DataTable" id="MNEN<%=sMgrSch[i][0]%> " onclick="ShowMenu(this, 0, '<%=sMgrTimOff[i]%>')" nowrap>
             <font color="<%=sMgrMltClr[i]%>"><%=sMgrSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfSls; i++){%>
         <tr>
          <td class="DataTable" id="SLEN<%=sSlsSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sSlsTimOff[i]%>')">
               <font color="<%=sSlsMltClr[i]%>"><%=sSlsSch[i][0]%>
       ...
       <td class="DataTable17" colspan="8">&nbsp;
          <a href="EmpNumbyHourWk.jsp?STORE=<%=sStore%>&STRNAME=<%=sThisStrName%>&MONBEG=<%=sMonth%>&WEEKEND=<%=sWeekEnd%>&WKDATE=<%=sWkDate[0]%>&FROM=<%=sFrom%>&WKDAY=<%=sDaysOfWeek[0]%>&SHWGRP=SLSBK">
      ...
      <!-- --------------- SLSP - Bike Department Details ---------------- -->
         ...
         <tr>
          <td class="DataTable" id="SBEN<%=sSBkSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sSBkTimOff[i]%>')">
               <font color="<%=sSBkMltClr[i]%>"><%=sSBkSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfNSl; i++){%>
         <tr>
          <td class="DataTable" id="NCEN<%=sNSlSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sNSlTimOff[i]%>')">
              <font color="<%=sNSlMltClr[i]%>"><%=sNSlSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfNRc; i++){%>
         <tr>
          <td class="DataTable" id="NREN<%=sNRcSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sNRcTimOff[i]%>')">
              <font color="<%=sNRcMltClr[i]%>"><%=sNRcSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfNBk; i++){%>
         <tr>
          <td class="DataTable" id="NBEN<%=sNBkSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sNBkTimOff[i]%>')">
              <font color="<%=sNBkMltClr[i]%>"><%=sNBkSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfNOt; i++){%>
         <tr>
          <td class="DataTable" id="NOEN<%=sNOtSch[i][0]%>" onclick="ShowMenu(this, 0, '<%=sNOtTimOff[i]%>')">
              <font color="<%=sNOtMltClr[i]%>"><%=sNOtSch[i][0]%>
      ...
      <%for(int i=0; i < iNumOfTrn; i++){%>
         <tr >
          <td class="DataTable" id="TREN<%=sTrnSch[i][0]%> " onclick="ShowMenu(this, 0, '<%=sTrnTimOff[i]%>')" nowrap>
             <font color="<%=sTrnMltClr[i]%>"><%=sTrnSch[i][0]%>
