<%@ page import="strempslsrep.StrProd2Dates, java.util.*"%>
<%
   ...
   StrProd2Dates rank = new StrProd2Dates(sStore, sFrom, sTo, sRank, sSlsPrsOnly, sNoDept);
...
function sbmNewWeek()
{
   ...
   SbmString = "StrProd2Dates.jsp?STORE=<%=sStore%>" + "&STRNAME=<%=sStrName%>"
             + "&FROMWKEND=" + selFrWeek
             + "&TOWKEND=" + selToWeek
             + "&RANK=<%=sRank%>"
             + "&SORT=<%=sSort%>"
 ...
 <table border="0" width="100%" height="100%" cellPadding="0" cellSpacing="0">
   <tr bgColor="moccasin">
     ...
     <tr bgColor="moccasin">
      ...
      <a href="StrProd2DatesSel.jsp"><font color="red" size="-1">Store Productivity Selection</font></a>&#62;
      <a href="StrProd2DatesSum.jsp?&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&SORT=<%=sSort%>&ONLY=<%=sSlsPrsOnly%>"><font color="red" size="-1">All Store Ranking</font></a>&#62;
      ...
      <br><font size="-1">Click here to show <%if(sSlsPrsOnly.equals("N")){%><a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&SORT=<%=sSort%>&ONLY=Y&NODEPT=<%=sNoDept%>">sales personnel only</a>;<%}
        else if(sSlsPrsOnly.equals("Y")){%><a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&SORT=<%=sSort%>&ONLY=N&NODEPT=<%=sNoDept%>">all employees included</a>;<%}%></font>
      &nbsp;&nbsp;&nbsp;&nbsp;
      <font size="-1">Click here to show <%if(sNoDept.equals("N")){%><a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&SORT=<%=sSort%>&ONLY=<%=sSlsPrsOnly%>&NODEPT=Y">w/o Department Totals</a>;<%}
        else {%><a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&SORT=<%=sSort%>&ONLY=<%=sSlsPrsOnly%>&NODEPT=N">with Department Totals</a>;<%}%></font>
...
<!-------------------------------------------------------------------->
  ...
  <table class="DataTable" cellPadding="0" cellSpacing="0">
    ...
    <tr>
      ...
      <th class="DataTable">LY</th>
      <th class="DataTable">
         <a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=SLSHR&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>&NODEPT=<%=sNoDept%>">TY</a></th>
      <th class="DataTable">LY</th>
      <th class="DataTable">TY</th>
      <th class="DataTable">LY</th>
      <th class="DataTable">
        <a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=SLSTR&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>&NODEPT=<%=sNoDept%>">TY</a></th>
      <th class="DataTable">LY</th>
      <th class="DataTable">TY</th>
      <th class="DataTable">LY</th>
      <th class="DataTable">
        <a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=ITMTR&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>&NODEPT=<%=sNoDept%>">TY</a></th>
      <th class="DataTable">LY</th>
      <th class="DataTable">TY</th>
      <th class="DataTable">LY</th>
      <th class="DataTable">
        <a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=AMT&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>&NODEPT=<%=sNoDept%>">TY</a></th>
      <th class="DataTable">LY</th>
      <th class="DataTable">
        <a href="StrProd2Dates.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=OVERALL&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>&NODEPT=<%=sNoDept%>">TY</a></th>
      <th class="DataTable">LY</th>
    </tr>
     ...
     <%for(int j=0; j < iNumOfEmp[i]; j++){%>
         <tr class="DataTable">
           ...
           <td class="DataTable1" nowrap>
              <a href="StrProd2DatesWk.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&EMP=<%=sEmp[i][j]%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&ONLY=<%=sSlsPrsOnly%>&RANK=<%=sRank%>&TyRank1=<%=sEmpTYRank1[i][j]%>&LyRank1=<%=sEmpLYRank1[i][j]%>&TyRank2=<%=sEmpTYRank2[i][j]%>&LyRank2=<%=sEmpLYRank2[i][j]%>&TyRank3=<%=sEmpTYRank3[i][j]%>&LyRank3=<%=sEmpLYRank3[i][j]%>&TyRank4=<%=sEmpTYRank4[i][j]%>&LyRank4=<%=sEmpLYRank4[i][j]%>&TyRank5=<%=sEmpTYRank5[i][j]%>&LyRank5=<%=sEmpLYRank5[i][j]%>&SORT=<%=sSort%>">
              <%=sEmpName[i][j]%></a></td>
...
<!------------------------------- Department Totals---------------------------->
    <%if(!sNoDept.equals("Y")) {%>
      <tr class="DataTable1">
         <td class="DataTable1" colspan="2" nowrap>
           <a href="StrProdDeptWk.jsp?STORE=<%=sStore%>&STRNAME=<%=sStrName%>&DPT=<%=sDpt[i]%>&FROMWKEND=<%=sFrom%>&TOWKEND=<%=sTo%>&RANK=<%=sRank%>&ONLY=<%=sSlsPrsOnly%>&SORT=<%=sSort%>">
              Dept: <%=sDpt[i]%> - <%=sDptName[i]%></a></td>
         <th class="DataTable1">&nbsp;</th>
