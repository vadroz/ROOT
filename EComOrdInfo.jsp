<%@ page import="ecommerce.EComOrdInfo, ecommerce.EComOrdAdjInfo"%>
<%   
...
{
   response.sendRedirect("SignOn1.jsp?TARGET=EComOrdInfo.jsp&APPL=ALL&" + request.getQueryString());
}
else
{
    ...
    EComOrdAdjInfo ordadj = new EComOrdAdjInfo(sSite, sOrder, sUser);
...
//==============================================================================
function sendCustInfoToBronto(site, ord, action)
{
   var url = "EComSendToBronto.jsp?Site=" + site
      + "&Order=" + ord
      + "&From=" + action
...
// send customer info to elab
//==============================================================================
function updateOrderProperties(group, site, tag, custid)
{
   var url = "EComVolAPIProd.jsp?Group=" + group
      + "&Site=" + site
      + "&" + tag + "=" + custid
...
function popAdjust(ord)
{
   ...
   panel += "<tr><th class='DataTable' rowspan=2 nowrap >Select</th>"
         ...
         + "</tr>"
         + "<tr>"
             + "<th class='DataTable' nowrap>Q-ty</th>"
      ...
      for(var i=0; i < OrdSku.length; i++)
      {
         ...
         panel += "<td class='Prompt' nowrap><input class='Small' name='SelItm' type='checkBox' onclick='enableDtl(this)'  value='" + i + "'></td>"
       ...
       <table border=1 cellPadding="0" cellSpacing="0" >
         <tr class="DataTable1">
           ...
           <td class="DataTable"><%=sOrd%>&nbsp;
              <%if(bAllowButton){%>
                 <button class="Small" onclick="updateOrderProperties('ORDERSNG','<%=sSite%>', 'Order', '<%=sOrd%>')">Get Order</button>
              <%}%>
           </td>
           ...
           <td class="DataTable"><%=sCust%> &nbsp;
               <%if(bAllowButton){%>
                   <button class="Small" onclick="updateOrderProperties('CUSTEMAIL','<%=sSite%>', 'CustomerId','<%=sCust%>')">Get EMail </button>
               <%}%>
           </td>
         </tr>
         <tr class="DataTable1">
...
<!-- ======================================================================= -->
       <table border=1 cellPadding="0" cellSpacing="0" >
         <tr class="DataTable">
             ...
             <th class="DataTable">Q-ty</th>
             <th class="DataTable">Retail</th>
             <th class="DataTable">Total<br>Retail</th>
             <th class="DataTable">Free<br>Ship</th>
             <th class="DataTable">Q-ty On<br>Hold</th>
             <th class="DataTable">Q-ty On<br>Shipping</th>
...
<br><br>
     ...
     <table border=1 cellPadding="0" cellSpacing="0" >
         <tr class="DataTable">
           <th class="DataTable" colspan=11>Alloction and Pick Distribution</th>
         </tr>
         <tr class="DataTable">
           <th class="DataTable">Sku</th>
           <th class="DataTable">Description</th>
        ...
        <%for(int i=0; i < iNumOfDtl; i++ )
        {
         ...
         %>
           ...
           %>
             <tr id="trItem" class="DataTable">
               ...
               <td class="DataTable2" nowrap><%=sAlloc[j]%></td>
               <td class="DataTable2" nowrap><%=sPick[j]%></td>
         ...
         <%}%>
               ...
               <td class="DataTable2" nowrap><%=sUps[j]%></td>
               <td class="DataTable2" nowrap><%=sPrtDate[j]%> <%=sPrtTime[j]%></td>
               <td class="DataTable2" nowrap><%=sShipMeth[j]%></td>
               <%if(bAllowDist){%>
      ...
      <%if(!sEmail.equals("Not found") && bAllowButton){%>
         &nbsp; &nbsp; &nbsp;
         <button class="Small" onclick="sendCustInfoToBronto('<%=sSite%>', '<%=sOrder%>', 'CSTINFO')">Send custormer information<br>to Bronto</button>
      <%}%>
      <%if(bUps){%>
          &nbsp; &nbsp; &nbsp;
     ...
     <table border=1 cellPadding="0" cellSpacing="0" >
      ...
      <%if(bAllowButton){%>
          ...
          <%if(!sEmail.equals("Not found")){%>
             &nbsp; &nbsp; &nbsp;
             <button class="Small" onclick="sendCustInfoToBronto('<%=sSite%>', '<%=sOrder%>', 'STSORD')">Send ship-to-store<br>status to Bronto</button>
          <%}%>
      <%}%>
