...
function clcNewPlan()
{
   ...
   // remove total inputs
   for(var i=0; i<NumOfVal; i++){TotSlsInp[i].value = ""; TotMkdInp[i].value = ""; TotInvInp[i].value = "";}
   ...
   for(var i=0; i < NumOfMon; i++)
   {
     for(var j=0; j < NumOfVal; j++)
     {
       if(AlwChg==Values[i])
       {
         if(SlsInp[i].value.trim()!="") setNewPlan(SlsInp[i].value.trim(), SlsCell, PlanSls, SlsCell);
         if(MkdInp[i].value.trim()!="") setNewPlan(TotMkdInp[i].value.trim(), MkdCell, PlanMkd, TotMkdCell);
         if(InvInp[i].value.trim()!="") setNewPlan(TotInvInp[i].value.trim(), InvCell, PlanInv, TotInvCell);
       }
     }  
   }
