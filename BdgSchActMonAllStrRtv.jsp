<%@ page import="java.util.*, java.text.*, payrollreports.BdgSchActMonAllStr"%>
<%
...
{
   response.sendRedirect("SignOn1.jsp?TARGET=BdgSchActMonAllStr.jsp&APPL=" + sAppl + "&" + request.getQueryString());
}
else
{
    BdgSchActMonAllStr bdgwkall = new BdgSchActMonAllStr(sYear, sMonth, sUser);
...
function bodyLoad()
{
   ...
   parent.MrqWait.style.display = "none"
}
//==============================================================================
...
function popGrpBdgPanel(str)
{
  ...
  panel += "<tr class='DataTable2'>"
             ...
             + "<th class='DataTable3' rowspan=2 nowrap>&nbsp</th>"
             + "<th class='DataTable3' colspan=7 nowrap>Sales<br>Retail</th>"
         + "</tr>"
         + "<tr class='DataTable2'>"
  ...
  for(var  i=0, j=0; i < GrpBdg[str].length; i++)
  {
       ...
       panel += "<td class='DataTable1' nowrap>" + GrpBdgName[str][i] + "</td>"
             ...
             + "<td class='DataTable2' nowrap>$0</td>"
...
<!-------------------------------------------------------------------->
      ...
      <table border=1 cellPadding="0" cellSpacing="0" >
        <tr>
          <th colspan=12></th>
          <th class="DataTable3" colspan=18>Hourly Employee Only (Excludes Holiday, Sick, Vacation and TMC.)</th>
        </tr>
        <tr>
          <th colspan=3></th>
          ...
          <th class="DataTable4" colspan=2>Hourly Rate</th>
          <th></th>
        ...
        <tr>
          ...
          <th class="DataTable2">Hours Earned (Based on Sales)</th>
          <th class="DataTable2">Allowable Budget Hours (Based on Sales)</th>
          <th class="DataTable2">Actual Hrs. / Adjusted Hrs. To Reach Allowable Budget</th>
          <th class="DataTable2">Actual Hrs. / Adjusted Hrs. To Reach Budget</th>
          <th class="DataTable2">Hours Actual / Scheduled</th>
          <th class="DataTable" rowspan=2>&nbsp;</th>
          <th class="DataTable2">Original Payroll Budget Dollars </th>
          <th class="DataTable2">Allowable Budget Dollars (Based on Sales)</th>
     ...
     <tr class="Divdr1"></td><td colspan=36>&nbsp;</td></tr>
     <!------------------------- Region --------------------------------------->
     <%for(int i=0; i < iNumOfReg; i++){%>
        ...
        %>
            ...
            <td class="DataTable"><a href="BdgSchActMn.jsp?Store=<%=sStr[iStrNum]%>&Year=<%=sYear%>&Month=<%=sMonth%>" target="_blank"><%=sStr[iStrNum]%></a></td>
            <th class="DataTable">&nbsp;</th>
            <%for(int k=0; k < sLineCell.length - 4; k++){%>
               <td class="DataTable<%if(k==6){%>01<%} else if(k==9 || k == 10 || k == 20 || k == 21){%>02<%}%>" nowrap>
                  <%if(k >= 0 && k <= 5 && k != 1 || k >= 12 && k <= 16 || k == 18 || k==24 || k==25){%>$<%}%><%=sLineCell[k]%><%if( k==1 || k >= 19 && k <= 21){%>%<%}%>
               </td>
               <%if(k == 5 || k == 11 || k == 16 || k == 21){%><th class="DataTable">&nbsp;</th><%}%>
            ...
            <script>
               ...
               ActEmpGrpSlsRet[NumOfGrp] = [<%=sActEmpGrpSlsRet%>];
               NumOfGrp++;
            </script>
           </tr>
        <%}%>
