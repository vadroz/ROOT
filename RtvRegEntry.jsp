...
function Validate(skuOrDoc)
{
  ...
  if(reason==null || reason=="NONE"){ msg += "Please select Reason.\n"; error = true; }

  if (error) alert(msg);
  else
  {
	  var arrsear = new Array();
	  arrsear[arrsear.length] = search;
	  var arrreas = new Array();
	  arrreas[arrreas.length] = reason;
	  var arrcommt = new Array();
	  arrcommt[arrcommt.length] = comment;
	  var arrdef = new Array();
	  arrdef[arrdef.length] = defect;
	  
	  saveRtventry(str, strnm, arrsear, arrreas, arrcommt, arrdef, skuOrDoc);
  }
}
//-------------------------------------------------------------
...
{      
	...
	var html = "<form name='frmAddSku'"
  	+ " METHOD=Post ACTION='RtvRegSave.jsp'>"
  	+ "<input class='Small' name='Store'>"
  	+ "<input class='Small' name='StrName'>"
  	;
  	
  	for(var i=0; i < search.length; i++)
  	{
  		html += "<input class='Small' name='Search'>"
	...
	window.frame1.document.all.Store.value = str;
	window.frame1.document.all.StrName.value = strnm;
	
	for(var i=0; i < search.length; i++)
  	{
		if(search.length == 1)
		{
			window.frame1.document.all.Search.value = search[i];
			window.frame1.document.all.Reason.value = reason[i];
			window.frame1.document.all.Defect.value = defect[i];
			window.frame1.document.all.Comment.value = comment[i];
		}
		else
		{
			window.frame1.document.all.Search[i].value = search[i];
			window.frame1.document.all.Reason[i].value = reason[i];
			window.frame1.document.all.Defect[i].value = defect[i];
	...
	var action = "ADD";
	var seq = "0";
	if(skuOrDoc && AddUpd==0) { action = "ADD"; seq = "0"; }
	else if(skuOrDoc && AddUpd==1) { action = "UPD"; seq = SaveSeq; }
	else if(skuOrDoc && AddUpd==2) { action = "DLT"; seq = SaveSeq; }
	else { action = "POST"; seq = "0"; }
...
function rtvItem(str, sku, seq, reason, comment, defect, row, action)
{
   ...
   else
   {
      for(var i=0; i < reasLst.length; i++)
      {
    	 ...
    	 if( reasLst.options[i].text == reason )
         {
...
function getVenSku()
{
	...
	if(ven != "" && ven != "ALL")
	{
		...
		document.all.trSelTopReas.style.display = "none";
		document.all.trSelTopDefect.style.display = "none";
	}
...
//get sku for selected vendor
//==============================================================================
function showVenSku(cls, ven, sty, clr, siz, desc, vst, sku, inv, upc)
{
	var hdr = "Item List";
	   ...
	   var html = "<table border=0 width='100%' cellPadding=0 cellSpacing=0>"
	     + "<tr>"
	       ...
	       + "</td></tr>"
	    + "<tr><td class='Prompt' colspan=2>" + popItem(cls, ven, sty, clr, siz, desc, vst, sku, inv, upc)
	     + "</td></tr>"
	   + "</table>"
...
//==============================================================================
function popItem( cls, ven, sty, clr, siz, desc, vst, sku, inv, upc )
{
    ...
    for(var i=0; i < sku.length; i++)
    {
    	panel += "<tr class='DataTable'>"
    	  + "<td class='DataTable' >" 
    	  	...
    	  	+ " onclick='if(this.checked){setItmReas(&#34;" + i + "&#34;)}'>"
    	  + "</td>"
    	  + "<td class='DataTable' >" + cls[i] + "-" + ven[i] + "-" + sty[i]  + "-" + clr[i]  + "-" + siz[i] + "</td>"
    	  + "<td class='DataTable' >" + sku[i] + "</td>"
    	  + "<td class='DataTable' >" + upc[i] + "</td>"
    	  + "<td class='DataTable' >" + desc[i] + "</td>"
    	  + "<td class='DataTable' >" + vst[i] + "</td>"
    	  + "<td class='DataTable' >" + inv[i] + "<input name='Inv"+ i + "' value='" + inv[i] + "' type='hidden'></td>"
    	  + "<td class='DataTable' ><input name='Qty" + i + "' value='1' size=5 maxlength=3 readonly"    	  
    	  ;
    	  ...
    	  panel += ">&nbsp;</td>"
    		...
    		+ "</td>"
    		+ "<td class='DataTable' ><input name='SelItmDef" + i + "' size=1 maxlength=1 readonly></td>"
    		+ "<td class='DataTable' ><input name='SelItmCommt" + i + "' size=12 maxlength=50 readonly readonly>"
    	   + "</tr>";
    }
    
              
  	panel += "<tr class='DataTable'>" 
  	   + "<td class='DataTable3' colspan=10><br><br><button onClick='vldVenSku(&#34;" + sku.length + "&#34;)' class='Small'>Submit</button>&nbsp;"
...
function vldVenSku(numofsku)
{
	...
	for(var i=0; i < numofsku; i++)
	{
		...
		if( skuobj.checked )
		{
			...
			var defobj = document.all[defobjnm];
			var commtnm = "SelItmCommt" + i; 
			var commtobj = document.all[commtnm];
			
			for(var j=0; j < qty[qty.length-1];j++)
			{
				sku[sku.length] = skuobj.value;
				reason[reason.length] = reasobj.value;			
	...
	{ 
		hidePanel();
		saveRtventry(str, strnm, sku, reason, comment, defect, true); 
	}
}
//==============================================================================
//hide panel
...
function hidePanel()
{
	...
	document.all.trSelTopReas.style.display = "block";
	document.all.trSelTopDefect.style.display = "block";
}
//==============================================================================
// set Item reason
//==============================================================================
...
function popItmReas(i)
{
	...
	panel += "<tr class='DataTable2'>"
   		...
   		+ "<td class='DataTable' ><select name='SelReas' onchange='chgItmSelReas(this)'></select></td>"   		
 	 + "</tr>"
 	 + "<tr class='DataTable2'>"
		+ "<td class='DataTable' >Defective Item:</td>"
		+ "<td class='DataTable' ><input name='SelDefect' type='checkbox' value='Y'></td>"   		
	 + "</tr>"
	 + "<tr class='DataTable2'>"
		+ "<td class='DataTable' >RTV Qty:</td>"
		+ "<td class='DataTable' ><input name='SelQty' value='1' size=5 maxlength=3></td>"   		
	 + "</tr>"
	 + "<tr class='DataTable2' id='trCommt'>"
		+ "<td class='DataTable' >Comment:</td>"
		+ "<td class='DataTable' ><input name='SelCommt' maxlength='50' size='50'></td>"   		
	 + "</tr>"
	 ; 
            
	panel += "<tr class='DataTable2'>" 
	   + "<td class='DataTable3' colspan=7><br><br><button onClick='setSelIImReas(&#34;" + i +"&#34;)' class='Small'>Set</button>&nbsp;"
...
function setSelIImReas(i)
{
	...
	var reas = "selItmReas" + i;
 	document.all[reas].value = document.all.SelReas.options[document.all.SelReas.selectedIndex].value;
 	var reas = "selItmReasNm" + i;
 	document.all[reas].value = document.all.SelReas.options[document.all.SelReas.selectedIndex].text;
	...
	var qty = "Qty" + i;
	document.all[qty].value = document.all.SelQty.value;
    ...
    <TD vAlign=top align=middle><B>Retail Concepts Inc.
      ...
      <TABLE border=0>
        ...
        <TR>
          ...
          <TD align=left colspan="3" nowrap>
             <input name="Search" maxlength=12; size=12">
...
<!-- ======================================================================= -->
     <div style="border-style: inset; height: 500px; overflow: auto;">
       <table border=1 cellPadding="0" cellSpacing="0" id="tbRtvEnt">
         <tr  class="DataTable">
            <th class="DataTable" colspan="13">Current Entry</th>
         </tr>
