...
{	
	...
	if(sUser.equals("vrozen") || sUser.equals("psnyder")
  	  ...
  	  || sUser.equals("satwood") || sUser.equals("dsimpson") || sUser.equals("mhodge")
  	  || sUser.equals("kknight")){sUserAuth = "DM";}  
...
<!------------------------------- order/sku --------------------------------->
           ...
           <!-- =========== Region Total ============= -->
           <tr class="Divider"><td colspan=22>&nbsp;</td></tr>
           <%
 ...
 <%if(sGrp.equals("Str") && sSelSts[0].equals("Processed")
        ...
        <tr class="trHdr01">
        	<th class="th02" width="5%" rowspan=3>Month</th>
        	<%for(int i = 0; i < iNumOfReas; i++){%>
        		<th class="th02"<%if(i == 6){%>colspan=5<%}%>><%=i+1%></th>
        	<%}%>
        	<th class="th02" width="5%" rowspan=3>Total
        	 	<br><%if(sType.equals("U")){%>Units<%}
        	 		else if(sType.equals("C")){%>Cost<%}
        	...
        	<%if(sDispStrAppr.equals("1")){%>
        	   <th class="th02" colspan=2 rowspan=3>&nbsp;</th>
        	<%}%>
        </tr>
        </tr>
        <tr class="trHdr01">
        	<%for(int i = 0; i < iNumOfReas; i++){%>
        		<th class="th02" width="6%" <%if(i != 6){%>rowspan=2<%} else{%>colspan=5<%}%>>
        ...
        <tr class="trHdr01">
        	<%int max = Integer.parseInt(sRsScMax[6]);       		     
       		   for(int j = 0; j < max; j++){%>
          	        <th class="th19" id="thYReasL3<%=6%><%=j%>"><%=sRsScList[6][j]%></th>
        	   <%}%> 	
        	  <th class="th19">Total</th> 
        </tr>
    		...
    		<tr id="trTot" class="<%=sYtdRowCls%>">
             ...
             <%for(int j = 0; j < iNumOfReas; j++){%>
                <%if(j==6){
                	if(i < iNumOfMon) 
                	{ 
                		ctlinfo.setCatSum2(sReasLst[j], String.valueOf(i+1));
                	}
        			...
        			for(int k = 0; k < max; k++){
        				...
        				{ 
        					ctlinfo.setCat();
        					System.out.println("1.   i=" + i + " max=" + iNumOfMon);
        				} 
        				else
        				{ ctlinfo.setTotCat(j, sRefresh); 
        				  sRefresh = " ";   
        				  System.out.println("2.   i=" + i + " max=" + iNumOfMon);
        				}
					...
					%>						
          	        	<td id="tdYCat<%=i%><%=j%><%=k%>" class="td11" nowrap><%=sCatAmt%></td>
        			<%}%>
        	    <%}%>
        		<td class="td12" nowrap>
