...
{
     response.sendRedirect("SignOn1.jsp?TARGET=RentInvListPiWiSel.jsp&APPL=ALL");
}
else
{
...
function Validate(dwnl)
{
  ...
  var yrobj = document.getElementsByName("selYear")[0];
  var year = yrobj.options[yrobj.selectedIndex].value;

  if (error) alert(msg);
  else{ sbmPlan( cls, str, year ) }
  return error == false;
}
//==============================================================================
// Submit OTB Planning
//==============================================================================
function sbmPlan(cls, str, year)
{
  var url = null;
  url = "RentInvListPiWi.jsp?"
...
<TABLE height="100%" width="100%" border=0>
  ...
  <TR bgColor=moccasin>
    <TD vAlign=top align=middle>
        ...
        <BR>Rental Inventory List Count Reconcilliation</B>
    ...
    <TR>
       ...
       <%}%>
        ...
        <TR>
          <TD class="Cell3" colspan=5>Fiscal Year: &nbsp;
          <select name="selYear">
          
          <%
       sStmt = "select pyr#"
        + " from RCI.FsyPer"
        + " where pyr# >= year(current date) - 4 and pyr# <= year(current date) + 1"
        + " group by pyr#"
        + " order by pyr# desc"
       ;
        ...
        while(runsql.readNextRecord())
        {
           String sYear = runsql.getData("pyr#").trim();
       %>
