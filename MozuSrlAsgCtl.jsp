<!DOCTYPE html>
<%@ page import="mozu_com.MozuSrlAsgCtl, inventoryreports.PiCalendar
...
<%
    ...
    String sInDiv = request.getParameter("InDiv");
    ...
    if(sInDiv==null){ sInDiv = "No"; }
//----------------------------------
// Application Authorization
//----------------------------------
if (session.getAttribute("USER")==null)
{
   response.sendRedirect("SignOn1.jsp?TARGET=MozuSrlAsgCtl.jsp&APPL=ALL&" + request.getQueryString());
}
else
{
	String sStmt = null;
	...
	{
		sSelSite = runsql.getData("BxSite");
	}	
	rs.close();
	runsql.disconnect();
    ...
    MozuSrlAsgCtl itmasgn = new MozuSrlAsgCtl(sStatus, sStsFrDate, sStsToDate
    ...
    boolean bStandAlone = !sInDiv.equals("Yes");
    
    boolean bAlwGift = sUser.equals("vrozen")  || sUser.equals("psnyder");
...
<style>body {background:ivory;}
        ...
        table.DataTable { order-bottom: grey solid 1px; border-left: grey solid 1px; padding: 0px;
              border-spacing: 0; border-collapse: collapse; width:100%; 
              border: grey solid 1px; font-size:10px }
        table.DataTable1 {  order-bottom: grey solid 1px; border-left: grey solid 1px; padding: 0px;
             border-spacing: 0; border-collapse: collapse; width:100%; 
        ...
        th.DataTable { border-bottom: grey solid 1px; border-left: grey solid 1px; 
        ...
        th.DataTable1 { cursor:hand; color: blue; background:#FFCC99;padding- top:3px; 
                       border-bottom: grey solid 1px; border-left: grey solid 1px;
                       padding-bottom:3px; text-align:center; font-size:11px; text-decoration: underline;}                       
        th.DataTable2 {border-bottom: grey solid 1px; border-left: grey solid 1px; 
                       padding-top:3px; padding-bottom:3px;text-align:left; vertical-align:top ;font-size:12px}               
        ...
        td.DataTable11 { border-bottom: grey solid 1px; border-left: grey solid 1px;background: pink; padding-top:3px; padding-bottom:3px; padding-left:3px;
                       padding-right:3px; text-align:left; white-space:}
        td.DataTable12 { background: lightgreen; padding-top:3px; padding-bottom:3px; padding-left:3px;
                       padding-right:3px; text-align:left; white-space:}
        ...
        div.dvProgBar { position:absolute; visibility:hidden; background-attachment: scroll;
              border: black solid 2px; width:250; height:50px; background-color:azure; z-index:10;
...
{	
  ...
  document.all.trErrCode.style.display="none";  
...
function popQtySel(site, ord, sku, snid, str, sts, qty, availqty,  action)
{
   ...
   var panel = "<table cellPadding='0' cellSpacing='0'>"
    ...
    + "<tr id='trErrCode'>"
    	+ "<td nowrap class='Small'>Error Code: </td>"      
    	+ "<td nowrap>"
       		+ "<select class='Small' name='SelErrCode'></select>"
    	+ "</td>"
  	+ "</tr>"
  ...
  panel += "<tr>"
    ...
    + "<tr>"
         + "<td nowrap class='Small' colspan=2><button id='btnChgSts' onClick='ValidateStrPickSendQty("
            + "&#34;" + site + "&#34;,&#34;" + ord + "&#34;,&#34;" + sku + "&#34;"
            + ",&#34;" + snid + "&#34;"
            + ",&#34;" + str + "&#34;"            
            + ",&#34;" + qty + "&#34;"
...
//set error code list 
//==============================================================================
function setErrCodeList()
{
	var errcode = document.all.SelErrCode;
	var errlst = ["WPS","DMG","SOP","DEF","TAG","EXT","OTHER"];
	
	errcode.options[errcode.length] = new Option("--- select error code ---", "NONE");	
	for(var i=0; i < errlst.length;i++)
	{
	      errcode.options[errcode.length] = new Option(errlst[i], errlst[i]);	       	   
	}
}
//==============================================================================
...
function setStsAttr(sel)
{
	...
	{
		document.all.trErrCode.style.display="block";
	}
}
//==============================================================================
...
function ValidateStrPickSendQty(site,ord,sku, snid, str,qty,availqty,action)
{
    ...
    var errcode = document.all.SelErrCode.options[document.all.SelErrCode.selectedIndex].value;
    if(sts == "Error" && errcode=="NONE")
    {	
    	error = true; msg += "<br>Please, select Error Code";        	
    }
    if(errcode=="NONE"){errcode="";}
    ...
    else if(sts == "Error" && errcode=="OTHER" && note == ""){error = true; msg += "<br>Please type error explanation.";}

    if(error){ document.all.tdError.innerHTML=msg; }
    else
    {   
    	document.all.btnChgSts.disabled = true;
    	sbmStrPickSendQty(site, ord, sku, snid, str, sts, emp, reas, note, rslvty, qty,errcode, action); 
    }
}
//==============================================================================
...
//==============================================================================
function sbmStrPickSendQty(site,ord,sku, snid, str, sts, emp, reas, note, rslvty, qty, errcode, action)
{
    ...
    var html = "<form name='frmChgStrSts'"
       + " METHOD=Post ACTION='MozuSrlAsgSave.jsp'>"
       + "<input name='Site'>"
       + "<input name='Order'>"
       + "<input name='Sku'>"
       + "<input name='Str'>"
   ...
   if(isIE || isSafari)
   {
   		...
   		window.frame1.document.all.Note.value = "<REASON>" + reas + "</REASON>" 
    		+ "<ERRCODE>" + errcode + "</ERRCODE>" + note;
   ...
   else
   {
		...
		window.frame1.contentDocument.forms[0].Srn.value = snid;
		window.frame1.contentDocument.forms[0].Str.value = str;
		window.frame1.contentDocument.forms[0].FromStr.value = ReasgFromStr;
		window.frame1.contentDocument.forms[0].Sts.value = sts + rslvty;;
		window.frame1.contentDocument.forms[0].Emp.value = emp;
		window.frame1.contentDocument.forms[0].Qty.value = qty;

		if(rslvty == "CNF"){ window.frame1.contentDocument.forms[0].Excl.value = "Y"; }
		else { window.frame1.contentDocument.forms[0].Excl.value = "N"; }
		window.frame1.contentDocument.forms[0].Reas.value = reas;
	
		window.frame1.contentDocument.forms[0].Note.value = "<REASON>" + reas + "</REASON>" 
		+ "<ERRCODE>" + errcode + "</ERRCODE>" + note;
...
function popComment(site, ord, sku, serial, str, type,emp, commt, recusr, recdt, rectm)
{
   ...
   {
      panel += "<tr class='DataTable'><td class='DataTable' nowrap>" + serial[i] + "</td>"
      panel += "<td class='DataTable' nowrap>" + type[i] + "</td>"

      if(str[i] != "0") { panel += "<td class='DataTable'>" + str[i] + "&nbsp;</td>" }
...
function showFedEx(fxhdr, fxdtl)
{
	...
	var hdr = "Federal Express Information. Order: " + fxhdr[0];
	  ...
	  for(var i=0; i < fxdtl.length; i++)
	  {
		 ...
		 var track = "TrackId" + i;
		 if(fxdtl[i][3]=="SHIPPED"){ document.all[track].readOnly = false; }
		 else { document.all[track].readOnly = true; }
	  }
	}
	//==============================================================================
	// populate panel
	...
	panel += "<table border=1 style='font-size:12px' cellPadding='0' cellSpacing='0' width='100%' id='tblLog'>"
		    + "<tr class='DataTable'>"
		       ...
		       + "<th class='DataTable'>Tracknumber ID</th>"
		    + "</tr>";
	...
	for(var i=0; i < fxdtl.length; i++)
	{
		panel += "<tr class='DataTable'>"
		   ...
		   + "<td class='DataTable'>" + fxdtl[i][1] + "&nbsp;"
		     + "<input name='Sku" + i + "' value='" + fxdtl[i][1] + "' type='hidden'>"
		   + "</td>"
		   + "<td class='DataTable'>" + fxdtl[i][2]
		     + "<input name='SrlNum" + i + "' value='" + fxdtl[i][2] + "' type='hidden'>"
		   + "&nbsp;</td>"
		   + "<td class='DataTable'><select name='Status" + i + "' class='Small' onchange='setTrackIdCell(" + i +")'></select></td>"
		   ...
		   + "<td class='DataTable'>" + fxdtl[i][8] + "&nbsp;"
		   		+ "<input name='PackId" + i + "' value='" + fxdtl[i][8] + "' type='hidden'>"
		   + "</td>"
		   + "<td class='DataTable'><input name='TrackId" + i + "' class='Small' value='" + fxdtl[i][9] + "'></td>"
		 + "</tr>";
	}	    
...
//==============================================================================
function setTrackIdCell(i)
{
	var sts = "Status" + i;
	var obj = document.all[sts];
	var trackNm = "TrackId" + i;
	if(obj.options[obj.selectedIndex].value == "SHIPPED")
	{ 
		document.all[trackNm].readOnly = false;
		if(document.all[trackNm].value  == "NOT FOUND")
		{
		document.all[trackNm].value = "";
		}
	}
	else { document.all[trackNm].readOnly = true; }
}
//==============================================================================
// validate fed ex corrections
//==============================================================================
function vldFedEx()
{
    ...
    {
    	var srlnm = "SrlNum" + i;
    	srln[i] = document.all[srlnm].value;
    }
    ...
    {
    	var tracknm = "TrackId" + i;
    	track[i] = document.all[tracknm].value;
    }
    ...
    {
    	var packnm = "PackId" + i;
    	pack[i] = document.all[packnm].value;
    }
    
    if(error){ document.all.tdError.innerHTML=msg; }
    else{ sbmFedEx(cmpny, addrs1, addrs2, city, state, zip, phone, shpmeth, sku, sts, srln, pack, track); }
}
//==============================================================================
// submit store status changes
//==============================================================================
function sbmFedEx(cmpny, addrs1, addrs2, city, state, zip, phone, shpmeth, sku, sts, srln, pack, track)
{
	cmpny = cmpny.replace(/\n\r?/g, '<br />');
    ...
    var html = "<form name='frmChgFedEx'"
       + " METHOD=Post ACTION='MozuFedExInfo.jsp'>"
       + "<input name='Order'>"
       + "<input name='Company'>"
       + "<input name='Address1'>"
       + "<input name='Address2'>"
    ...
    for(var i=0; i < sku.length ;i++) { html += "<input name='Sku'>"; }
       for(var i=0; i < srln.length ;i++) { html += "<input name='Srl'>"; }
    for(var i=0; i < sts.length ;i++) { html += "<input name='Sts'>"; }
    for(var i=0; i < pack.length ;i++) { html += "<input name='Pack'>"; }
    for(var i=0; i < track.length ;i++) { html += "<input name='Track'>"; }
    ...
    { 
    	...
    	window.frame1.document.all.Srl.value = srln[0];
    	window.frame1.document.all.Track.value = track[0];
    	window.frame1.document.all.Pack.value = pack[0];
    }
    else 
    {
    	for(var i=0; i < sts.length ;i++)
    	{
       		...
       		window.frame1.document.all.Srl[i].value = srln[i];
       		window.frame1.document.all.Track[i].value = track[i];
       		window.frame1.document.all.Pack[i].value = pack[i];
    	}
    }
...
{
  url = "MozuSrlAsgCtl.jsp?Sort=" + sort
  for(var i=0; i < Sts.length;  i++)
  {
     url += "&Sts=" + Sts[i];
  }
...
function restart()
{
  url = "MozuSrlAsgCtl.jsp?Sort=<%=sSort%>"
...
{
  var url = "MozuOrdReprint.jsp?Site=" + site
      + "&Order=" + ord
...
//==============================================================================
function getSlsBySku(cls,ven,sty,sku)
{
  ...
  var panel = "<table border=1 cellPadding='0' cellSpacing='0' width='100%'>"
    ...
    for(var i=0; i < str.length; i++)
    {
      if( avl[i] != "0" || str[i] == snstr )
      {
         ...
         if(str[i] != snstr && snsts !="Open" && snsts != "Cannot Fill" && snsts != "Cancelled" && snsts != "Error")
         {
            panel += "<a href='javascript: setQtySel(&#34;" + site
                  + "&#34;,&#34;" + ord +  "&#34;, &#34;" + sku
                  + "&#34;,&#34;" + snid
...
{	
	progressTime++; 
	var html = "Please wait while S/N status is changing<br>"
...
<TABLE class="TblMainPage">
  ...
  <TR>
    <TD vAlign=top align=left><B>Retail Concepts Inc.
        ...
        <%if(bStandAlone){%><a href="../" class="small"><font color="red">Home</font></a>&#62;
        <a href="MozuSrlAsgCtlSel.jsp" class="small"><font color="red">Select</font></a>&#62;
...
<!-- ======================================================================= -->
       <table border=1 class="DataTable" cellPadding="0" cellSpacing="0">
         <tr class="DataTable" >
             ...
             <th class="DataTable" >SKU</th>
             <th class="DataTable" >Inv<br>Anl</th>
             
             <%if(bAlwGift) {%><th class="DataTable" >Gift<br>Card<br>Number</th><%}%>
       ...
       <%for(int i=0; i < iNumOfItm; i++ )
         {
            ...
            int iNumOfGft = itmasgn.getNumOfGft();
            String [] sGiftCard = itmasgn.getGiftCard();
       ...
       %>
         <tr class="DataTable0" <%if(!sOrdClr.equals("")){%>style="background:<%=sOrdClr%>"<%}%>>
            <td class="DataTable" nowrap rowspan="<%=iNumOfSn%>">
               <%if(bStandAlone){%><a href="MozuOrdInfo.jsp?Site=<%=sSite%>&Order=<%=sOrd%>" target="_blank"><%=sOrd%></a><%}
               else {%><%=sOrd%><%}%>
            </td>
            ...
            <td class="DataTable1<%=sClrSts%>" nowrap rowspan="<%=iNumOfSn%>"><%=sShipMethNm%></td>
            ...
            <td class="DataTable2" nowrap rowspan="<%=iNumOfSn%>"><a href="javascript: getSlsBySku('<%=sCls%>','<%=sVen%>','<%=sSty%>','<%=sSku%>')"><%=sSku%></a></td>
            <td class="DataTable2" nowrap rowspan="<%=iNumOfSn%>"><a href="MozuStockPriceAnlysis.jsp?Site=<%=sSite%>&Parent=<%=sCls%><%=sVen%><%=sSty%>&Sku=<%=sSku%>" target="_blank">Inv</a></td>
            
            <%if(bAlwGift) { String sBr = "";%><td class="DataTable" rowspan="<%=iNumOfSn%>">
                   <%for(int j=0; j < iNumOfGft; j++){%><%=sBr%><%=sGiftCard[j]%><% sBr = "<br>"; }%>
                </td>
            <%}%>
            ...
            <th class="DataTable" style="vertical-align:middle;" id="thLog<%=i%>" rowspan="<%=iNumOfSn%>">
                <%if(bStandAlone){%><a href="javascript: getFedEx('<%=sSite%>' , '<%=sOrd%>', '<%=sSku%>');">&nbsp;FX&nbsp;</a>
                <%} else {%>&nbsp;<%}%>
            </th>
            ...
            %>
                ...
                <td class="DataTable" id="tdTotAvl<%=i%>" nowrap> 
                    <%if(bStandAlone){%><a href="javascript: showAvlQty('<%=sSite%>','<%=sOrd%>','<%=sSku%>'
                    ,[<%=sStrJsa%>], '<%=sQty%>',[<%=sAvlJsa%>], document.all.tdTotAvl<%=i%>
                    , '<%=sSnId%>', '<%=sSnSts%>', '<%=sSnStr%>',[<%=sPlaceJsa%>])"><%=sTotAvl%></a><%} else {%>&nbsp; <%}%>
                </td>
                <td class="DataTable1" id="tdStrSts<%=i%>" nowrap><%=sSnStr%>-<%=sSnSts%> <%=sSnStsDt%></td>
                
                <td class="DataTable1" id="tdStrSts<%=i%>" nowrap>                   
                   <%if(bStandAlone){%><a href="javascript:setQtySel('<%=sSite%>', '<%=sOrd%>', '<%=sSku%>'
