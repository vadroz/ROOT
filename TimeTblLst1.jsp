...
<%@ page import="strschdinq.TimeTblLstMlt, strschdinq.SlsChartsMlt"%>

<% TimeTblLstMlt timtbl = null;
   SlsChartsMlt slschr = null;
   String sStore = request.getParameter("STORE");
   String sNumDays = request.getParameter("NUMDAYS");
   String sYear = request.getParameter("YEAR");
   String sMonth = request.getParameter("MONTH");
   String sDay = request.getParameter("DAY");


   timtbl = new TimeTblLstMlt(sStore, sYear, sMonth, sDay, "7");
   slschr = new SlsChartsMlt(sStore, sYear, sMonth, sDay, "7");
   ...
   String [] sAmount = null;
   String sOthAmt = slschr.getOthAmt();
   String sTotal = slschr.getSlsTotAmt();
 ...
 <html>
	 <head>
		...
		document.write("td.DataTable2 { border-bottom: lightgrey solid 1px; text-align:left; font-family:Arial; font-size:10px }");
                ...
                document.write("td.DataTable6 { border-left: darkred solid 1px; border-right: darkred solid 1px; border-bottom: darkred solid 1px; text-align:right; font-family:Arial; font-size:10px; }");
                document.write("td.DataTable7 { border-bottom: darkred solid 1px; text-align:right; font-family:Arial; font-size:10px; }");
    ...
    <table class="DataTable" cellPadding="0" cellSpacing="0" >
           ...
           <% for (int i=0; i < iEmpNum; i++){%>
             <tr>
              <%="<td class='DataTable1' nowrap bgcolor='#CCFFCC'>" + sEmpName[i] + "</td>"%>
              <%="<td class='DataTable' nowrap bgcolor='#CCFFCC'>" + sDept[i] + "</td>"%>
             </tr>

              <!--get date  -->
              <%sEmp = sEmpName[i].substring(0, 4);
                timtbl.setEmpInfo(sEmp);
                timtbl.getEmpWrkDates();
                iDate = timtbl.getDateNum();
                sDate = timtbl.getDates();
                sElapse = timtbl.getElapse();
                sAmount = slschr.getAmount(sEmp);%>
               <%for (int j = 0; j < iDate; j++){
                   sTime = timtbl.getHours(Integer.toString(j+1));%>
                <%="<tr><td class='DataTable' nowrap colspan='2'>" + sDate[j] + "</td>"%>
                   <%for (int k=0; k < 68; k++){%>
                     <%="<td class=DataTable2 bgcolor=" + sTime[k] + ">&#160;&#160;</td>"%>
                  <%}%>
                  <!-- Time at work-->
                  <%="<td class='DataTable6' nowrap bgcolor='lightgrey'>" + sElapse[j] + "</td>"%>
                  <!-- Employee Sales amount -->
              <%="<td class='DataTable7' nowrap bgcolor='lightgrey'>" + sAmount[j] + "</td>"%>
                  <%="</tr>"%>
                <%}%>
           <%}%>
           ...
           if(iSlsEmpNum > iEmpNum){%>
              <tr><td class="DataTable1" bgcolor="#CCFFCC" colspan="2">Other</td>
               <td colspan="68" class="DataTable2" bgcolor="white">&#160;</td>
               <td class="DataTable6" bgcolor="lightgrey">0:00</td>
               <td class="DataTable7" bgcolor="lightgrey"><%=sOthAmt%></td>
              </tr>
           <%}%>
           ...
           <tr><td class="DataTable1" bgcolor="cornsilk">Total</td>
               ...
               <td class="DataTable6" bgcolor="cornsilk"><%=timtbl.getElapseTot()%></td>
               <td class="DataTable7" bgcolor="cornsilk"><%=sTotal%></td>
              </tr>

        <!-- -----------------Positive block---------------------- -->
             <tr><td class="DataTable" rowspan="<%=iZeroBox%>"><%="Total Sales: " + sTotal%></td>
